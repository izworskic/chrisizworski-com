...
test('the planner is hidden behind one flag while the guide still works', () => {
  const js = fs.readFileSync(path.join(root, 'public/assets/isle-royale-map.js'), 'utf8');

  // One flag, off, and it puts the page in the state the CSS keys off.
  assert.match(js, /const PLANNER_ENABLED = false;/);
  assert.match(js, /if \(!PLANNER_ENABLED\) document\.body\.classList\.add\('planner-off'\)/);
  // Nothing may enter build mode while it is off: a map click would collect checkpoints against a
  // UI the visitor cannot see.
  assert.match(js, /route\.adding=PLANNER_ENABLED\?Boolean\(active\):false;/);
  // Every planner surface carries the class the flag hides, and the rule exists.
  ...
  // The runtime stays: turning the planner back on must be the flag, not a rebuild.
  assert.match(js, /async function resolveCanoeRouteAsync/);
  assert.match(js, /function autoPortageRouteCandidate/);
  assert.match(js, /function officialPortageReferenceGeometry/);
});
...
test('the map offers satellite imagery and a way past itself', () => {
  ...
  // On a phone the map swallows the drag, so getting to the guide cannot depend on a swipe.
  assert.match(html, /id="map-peek"/);
  assert.match(js, /document\.getElementById\('map-peek'\)\?\.addEventListener/);
  assert.match(html, /body\.planner-off \.map-wrap\{height:clamp\(380px,58dvh,560px\)/, 'the map must not fill a phone screen');
});
