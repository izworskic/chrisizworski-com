...
test('Mackinac page cache-busts planner asset and removes stale starting-city copy', () => {
  ...
  assert.match(html,/mackinac-island\.css\?v=20260920-live20/);
...
test('Mackinac first-screen route inputs are explicit and cache-safe', () => {
  ...
  assert.match(html,/data-mackinac-build="20260920-live20"/);
...
test('place intelligence is source-backed and never presented as live availability',()=>{
  ...
  assert.match(html,/20260920-live20/);
});
...
test('shareable Mackinac state restores inputs but never freezes live outputs',()=>{
  const html=fs.readFileSync(htmlPath,'utf8');
  const js=fs.readFileSync(jsPath,'utf8');
  assert.match(html,/mackinac-trip-state\.js\?v=20260920-live20/);
...
test('second Mackinac intent cohort remains distinct and conservatively seeded',()=>{
  const js=fs.readFileSync(jsPath,'utf8');
  const html=fs.readFileSync(htmlPath,'utf8');
  assert.match(js,/'accessibility':\{\}/);
  assert.match(js,/'bike-route':\{trip_vision:\['biking'\]\}/);
  assert.match(js,/'where-to-stay':\{\}/);
  assert.match(js,/'fall':\{trip_vision:\['scenery'\]\}/);
  for(const slug of ['accessibility','bike-route','where-to-stay','fall'])assert.match(html,new RegExp('/mackinac-island/'+slug+'/'));
  assert.match(html,/20260920-live20/);
});
