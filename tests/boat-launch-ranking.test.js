...
test('nothing beyond the road-distance cap is presented as a nearby choice', () => {
  ...
  const result = rank.finalizeShortlist({
    pool,
    nextStraightMiles,
    drive: { legs: [{ index: 0, meters: 95 * METERS, seconds: 120 * 60 }] },
  });
...
test('the pool widens when a launch outside it could still win on road distance', () => {
  ...
  const kept = rank.finalizeShortlist({
    pool,
    nextStraightMiles: 9,
    limit: 1,
    drive: { legs: [{ index: 0, meters: 40 * METERS, seconds: 60 * 60 }] },
  });
  assert.equal(kept.needsWiderPool, true, '9 straight-line miles can beat a 40 mile drive, so the pool must widen');

  const settled = rank.finalizeShortlist({
    pool,
    nextStraightMiles: 55,
    limit: 1,
    drive: { legs: [{ index: 0, meters: 12 * METERS, seconds: 20 * 60 }] },
  });
...
test('an unroutable coordinate is dropped rather than counted as zero miles', () => {
  ...
  const result = rank.finalizeShortlist({
    ...
    drive: { legs: [{ index: 1, meters: 6 * METERS, seconds: 10 * 60 }] },
  });
...
test('Great Lakes connecting rivers are readmitted despite the DNR access field', () => {
  const { WHERE, CONNECTING_WATERS, connectionBasis, eligibleAttributes } = launches._test;
  assert.ok(WHERE.includes("greatlakesaccess LIKE 'Yes%'"));
  assert.ok(WHERE.includes("waterbody IN ('Detroit River','Saint Clair River','Saint Marys River')"));
  ...
  const elizabethPark = {
    ...
    greatlakesaccess: 'No, greater than 2 miles or does not connect (inland lake, etc.)',
    flag: 'InProgress',
  };
  assert.equal(connectionBasis(elizabethPark), 'great-lakes-connecting-water');
  assert.equal(eligibleAttributes(elizabethPark), true);

  const inlandLake = { ...elizabethPark, waterbody: 'Houghton Lake' };
  assert.equal(connectionBasis(inlandLake), null);
  assert.equal(eligibleAttributes(inlandLake), false, 'inland lakes stay out of a Great Lakes finder');
});

test('an unlisted trailer-parking count is not treated as zero spaces', () => {
  assert.ok(
    /const listed=num\(a\.trailerParking\);\s*\n\s*if\(listed===null\)return false;/.test(finderJs),
    'parking refinement must handle an unlisted count explicitly'
  );
  assert.ok(/function parkingUnknown/.test(finderJs), 'hidden unlisted-parking records must be countable');
  assert.ok(/hidden by your refinements/.test(finderJs), 'refinements must report what they removed');
});
...
test('the finder ranks on the shared drive-distance core and caps its reach', () => {
  ...
  assert.ok(/MAX_ROAD_MILES/.test(finderJs));
  assert.ok(/mi by road/.test(finderJs), 'road distance must be labeled as road distance');
  ...
  assert.ok(
    finderHtml.includes('/assets/boat-launch-ranking.js'),
    'the page must load the ranking core before the finder'
  );
  assert.ok(
    finderHtml.indexOf('/assets/boat-launch-ranking.js') < finderHtml.indexOf('/assets/boat-launch-finder.js'),
    'the ranking core must load first'
  );
});

test('an out-of-scope destination is told it is out of scope', () => {
  assert.ok(/outOfScope/.test(finderJs));
  assert.ok(/Inland-lake launches are a separate DNR inventory/.test(finderJs));
  assert.ok(/reason:'out-of-scope'/.test(finderJs));
  assert.ok(/inland-lake launches are a separate DNR inventory/i.test(finderHtml));
});
