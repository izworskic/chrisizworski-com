...
function enrichPayload(payload){
  ...
  if(selected&&Array.isArray(payload.alternatives)){
    payload.alternatives=payload.alternatives.map(alt=>({...alt,whyNotSelected:alternativeReason(selected,alt,payload.input||{})}));
  }
  const sources=Array.isArray(payload.sources)?payload.sources:[];
  const nws=sources.find(source=>source?.name==="National Weather Service");
  if(nws){
    const weather=selected?.weather;
    const available=weather?.ok===true;
    nws.status=available?"live":"unavailable";
    nws.url=available&&weather?.source?weather.source:"https://www.weather.gov/";
    nws.updated=available&&weather?.updatedAt?weather.updatedAt:null;
    nws.note=available
      ?"Hourly forecast loaded for representative high terrain on the selected route."
      :"The selected route's NWS hourly forecast could not be loaded. No gateway-city forecast was substituted.";
  }
  return payload;
}
...
module.exports=async function blueRidgeParkway(req,res){
  ...
  res.json=payload=>originalJson(enrichPayload(payload));
