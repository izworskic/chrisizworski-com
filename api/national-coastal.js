...
module.exports = async function handler(req,res){
  ...
  const day1=a.status==="fulfilled"?a.value:null, day2=b.status==="fulfilled"?b.value:null, observation=c.status==="fulfilled"?c.value:null, tide=d.status==="fulfilled"?d.value:null;
  const sources=[sourceMeta({name:"NOAA/NWS Marine Beach Forecast Summary",url:NWS+"/0",updatedAt:day1&&day1.issued_at||day2&&day2.issued_at||null,staleAfterMinutes:1440,available:Boolean(day1||day2),status:a.status==="rejected"&&b.status==="rejected"?"unavailable":null}),sourceMeta({name:"NOAA/NDBC real-time station observations",url:NDBC_ACTIVE,updatedAt:observation&&observation.observed_at||null,staleAfterMinutes:90,available:Boolean(observation),status:c.status==="rejected"?"unavailable":null}),sourceMeta({name:"NOAA CO-OPS tide predictions",url:"https://tidesandcurrents.noaa.gov/",updatedAt:null,staleAfterMinutes:null,available:Boolean(tide&&tide.predictions&&tide.predictions.length),status:d.status==="rejected"?"unavailable":tide&&tide.predictions&&tide.predictions.length?"prediction":"unavailable"})];
  return res.status(200).json({retrieved_at:new Date().toISOString(),degraded:settled.some(function(x){return x.status==="rejected";}),coastal_available:Boolean(day1||day2||observation||tide&&tide.predictions&&tide.predictions.length),search_radius_miles:RADIUS,location:{latitude:lat,longitude:lon},decision:summary(day1,day2,observation,tide),official_beach_forecast:{day1:day1,day2:day2},nearby_observation:observation,tide_context:tide,sources:sources,limitations:["This tool does not produce a universal beach- or boating-safety score.","NWS rip/swim-risk forecasts apply to the named forecast area and do not replace local flags, closures, lifeguards or emergency instructions.","NDBC stations can be offshore or otherwise separated from the exact beach; distance is shown and observations are context, not on-site proof.","NOAA tide predictions are predictions referenced to the named station and datum; they are not observed water level and they do not apply to non-tidal Great Lakes water-level changes.","Missing data remains unavailable rather than being converted into a neutral or favorable value."]});
};
module.exports._test={beachUrl:beachUrl,summary:summary,miles:miles,normalizeBeach:normalizeBeach,obs:obs,coopsStations:coopsStations,ndbcStations:ndbcStations,ndbcRows:ndbcRows,risk:risk,tideUrl:tideUrl};
