...
const UA = "ChrisIzworskiNationalFrostPlanner/2.0 (+https://chrisizworski.com/national-tools/frost/)";
const TYPES = [
...
async function normals(lat, lon) {
  for (const span of [0.35, 0.8, 1.8]) {
    ...
    const stations = (Array.isArray(rows) ? rows : [])
      .map((r) => station(r, lat, lon))
      .filter((s) => s.dates.spring_50 || s.dates.spring_10 || s.dates.fall_50)
...
async function nws(lat, lon) {
  const p = await json(`https://api.weather.gov/points/${lat.toFixed(4)},${lon.toFixed(4)}`);
  const u = p?.properties?.forecastHourly;
  if (!u) return null;
  const h = await json(u);
...
module.exports = async function handler(req, res) {
  ...
  return res.status(200).json({
    ...
    location: { latitude: lat, longitude: lon, timeZone: weather?.timeZone || null },
...
module.exports._test = { freezeVerdict, haversine, normalizeDate, station };
