...
function popupLiveHtml(p,conditions,wx){
  const match=conditions?.error?null:nearestGauge(p,conditions.gauges);
  const g=match?.reading||null,fc=g?.flow_context||{},stats=g?.seasonal_stats||{};
  const now=wx?.error?null:(wx?.hourly?.[0]||wx?.forecast?.[0]||null);
  const alerts=wx?.error?[]:(wx?.alerts||[]);
  const riverRows=[];
  if(match){
    riverRows.push(`<li><span>Nearest gauge</span><b>${esc(match.meta.name)}</b><small>${match.distance.toFixed(1)} mi straight-line from this access</small></li>`);
    riverRows.push(`<li><span>Flow</span><b>${g?.discharge_cfs!=null?`${fmt(g.discharge_cfs,0)} cfs`:'Not reported'}</b><small>${fc.percent_of_median!=null?`${fc.percent_of_median}% of seasonal median`:esc(fc.label||'Seasonal comparison unavailable')}${stats.p50!=null?` · median ${fmt(stats.p50,0)} cfs`:''}</small></li>`);
    riverRows.push(`<li><span>Water / stage</span><b>${g?.water_temp_f!=null?`${fmt(g.water_temp_f,1)}°F`:'Temp not reported'} · ${g?.gage_height_ft!=null?`${fmt(g.gage_height_ft,2)} ft`:'stage not reported'}</b><small>${esc(g?.temperature_context?.label||'')}</small></li>`);
    if(g?.turbidity_fnu!=null||g?.dissolved_oxygen_mgl!=null)riverRows.push(`<li><span>Extra sensors</span><b>${g.turbidity_fnu!=null?`Turbidity ${fmt(g.turbidity_fnu,1)} FNU`:''}${g.turbidity_fnu!=null&&g.dissolved_oxygen_mgl!=null?' · ':''}${g.dissolved_oxygen_mgl!=null?`O₂ ${fmt(g.dissolved_oxygen_mgl,1)} mg/L`:''}</b></li>`);
    riverRows.push(`<li><span>Freshness</span><b>${esc(g?.fresh?'Live / recent':'Stale / verify')}</b><small>${esc(freshness(g))}</small></li>`);
  }else riverRows.push(`<li><span>River conditions</span><b>${conditions?.error?'USGS unavailable':'No active same-waterway gauge mapped'}</b></li>`);
  const weatherRows=[];
  if(now){
    weatherRows.push(`<li><span>Now</span><b>${now.temperature!=null?`${now.temperature}°${esc(now.temperatureUnit||'F')} · `:''}${esc(now.shortForecast||'Forecast available')}</b></li>`);
    weatherRows.push(`<li><span>Wind / precipitation</span><b>${esc(`${now.windDirection||''} ${now.windSpeed||'wind not reported'}`.trim())}</b><small>${wx.precipitation_context?.max_probability!=null?`${wx.precipitation_context.max_probability}% max precipitation · `:''}${esc(wx.precipitation_context?.label||'')}</small></li>`);
    weatherRows.push(`<li><span>NWS alerts</span><b>${alerts.length?esc(alerts[0].event||'Active alert'):'None returned for this point'}</b>${alerts.length>1?`<small>+${alerts.length-1} additional active alert${alerts.length>2?'s':''}</small>`:''}</li>`);
  }else weatherRows.push(`<li><span>Weather</span><b>${wx?.error?'NWS unavailable':'No forecast period returned'}</b></li>`);
  return `<div class="mrp-live-grid">
    <section><h4>River right now</h4><ul class="mrp-stat-list">${riverRows.join('')}</ul></section>
    <section><h4>Weather at this access</h4><ul class="mrp-stat-list">${weatherRows.join('')}</ul></section>
  </div><p class="mrp-caution">Gauge data describes the gauge location. NWS data is nearby land-weather context. Neither is a wading, paddling, boating or fishing-safety verdict.</p>`;
}
...
async function enrichPopup(id){
  const p=DATA.places.find(x=>x.id===id);if(!p)return;
  const target=document.querySelector(`[data-popup-live="${CSS.escape(id)}"]`);if(!target||target.dataset.loaded==='true'||target.dataset.loading==='true')return;
  target.dataset.loading='true';
  const [conditions,wx]=await Promise.all([getConditions(),getWeather(p)]);
  if(!target.isConnected)return;
  target.innerHTML=popupLiveHtml(p,conditions,wx);target.dataset.loaded='true';delete target.dataset.loading;
}
function enrichOpenPopups(){document.querySelectorAll('[data-manistee-popup]').forEach(card=>enrichPopup(card.dataset.manisteePopup));}
function observePopups(){
  document.addEventListener('manistee:popup-open',event=>enrichPopup(event.detail?.id));
  const pane=document.querySelector('.leaflet-popup-pane');if(!pane)return;
  const observer=new MutationObserver(()=>queueMicrotask(enrichOpenPopups));observer.observe(pane,{childList:true,subtree:true});enrichOpenPopups();
}
...
function addLegend(){
  ...
  key.innerHTML=`<strong>River key</strong>
    ...
    <small>Lines: USGS NHD · point popups show source, reach, live river and local weather context</small>`;
  wrap.appendChild(key);
}
function addStyles(){
  if($('#manistee-depth-styles'))return;const s=document.createElement('style');s.id='manistee-depth-styles';
  s.textContent=`.manistee-river-key{position:absolute;z-index:690;left:14px;bottom:58px;background:rgba(255,255,255,.96);border:1px solid #cfd5d0;border-radius:10px;padding:10px 11px;width:190px;box-shadow:0 3px 12px rgba(0,0,0,.14);font:11px/1.25 Inter,system-ui,sans-serif;color:#35423d}.manistee-river-key strong{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:7px}.manistee-river-key span{display:flex;align-items:center;gap:7px;margin:5px 0}.manistee-river-key small{display:block;color:#79817d;border-top:1px solid #e8e8e4;margin-top:7px;padding-top:6px}.mi-line{display:inline-block;width:24px;height:0;border-top:4px solid #0d5c63}.mi-line.pine{border-color:#2f7d32}.mi-line.companion{border-color:#76558f;border-top-style:dashed}.mi-dot{width:10px;height:10px;border-radius:50%;display:inline-block;border:1px solid #fff;box-shadow:0 0 0 1px #aaa}.mi-dot.access{background:#c56b28}.mi-dot.camp{background:#5d7046}.mi-dot.gauge{background:#2764a8}.manistee-intel{margin-top:18px;padding-top:17px;border-top:2px solid #d6dfda}.manistee-intel h3,.manistee-system-depth h3{font-family:Georgia,serif;font-size:20px;font-weight:500;margin:4px 0 11px}.mi-eyebrow{font-size:9px;text-transform:uppercase;letter-spacing:.1em;color:#a94f25;font-weight:800}.mi-block{border-top:1px solid #e8e4dc;padding-top:12px;margin-top:13px}.mi-block h4{font-size:12px;margin:0 0 8px;color:#25352e}.mi-metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.mi-metric{background:#f3f5f2;border-radius:7px;padding:8px;min-width:0}.mi-metric span{display:block;font-size:9px;text-transform:uppercase;letter-spacing:.05em;color:#7a837f}.mi-metric b{display:block;font-size:13px;margin-top:2px;color:#23332c;overflow-wrap:anywhere}.mi-metric small,.mi-source-line,.mi-muted{display:block;font-size:9px;color:#737d78;line-height:1.45;margin-top:3px}.mi-reach{font-size:11px!important;background:#faf6eb;border-left:3px solid #a94f25;padding:8px 9px;color:#56615c!important}.mi-gauge-head{display:flex;align-items:center;justify-content:space-between;gap:8px;font-size:11px}.mi-fresh{font-size:8px;text-transform:uppercase;border-radius:999px;padding:3px 6px}.mi-fresh.yes{background:#e2f1e4;color:#236335}.mi-fresh.no{background:#eeeae4;color:#73675a}.mi-hourly{display:flex;gap:5px;overflow:auto;margin:8px 0}.mi-hourly span{min-width:82px;background:#f3f5f2;border-radius:6px;padding:6px;font-size:9px}.mi-hourly b,.mi-hourly small{display:block}.mi-alerts{background:#fff4dc;border:1px solid #e7d39e;border-radius:7px;padding:8px;font-size:9px;line-height:1.45;margin-top:7px}.mi-alerts strong{display:block;margin-bottom:4px}.mi-block ul{padding-left:17px;margin:5px 0}.mi-block li{font-size:10px;line-height:1.45;margin:5px 0}.manistee-system-depth{margin-top:14px;padding:13px;background:#f7f5ef;border:1px solid #dedbd1;border-radius:10px}.mi-system-grid{display:grid;gap:7px;margin-top:9px}.mi-system-grid article{background:#fff;border:1px solid #e2dfd6;border-radius:7px;padding:8px}.mi-system-grid strong,.mi-system-grid span,.mi-system-grid small{display:block}.mi-system-grid strong{font-size:11px}.mi-system-grid span{font-size:10px;margin-top:3px}.mi-system-grid small{font-size:9px;color:#737d78;margin-top:2px}.manistee-rich-popup .leaflet-popup-content-wrapper{border-radius:10px;box-shadow:0 8px 28px rgba(24,39,32,.22);overflow:hidden}.manistee-rich-popup .leaflet-popup-content{margin:0;width:auto!important;font-family:Inter,system-ui,sans-serif}.mrp-card{padding:13px 14px 12px;color:#2a3932;line-height:1.35}.mrp-card h3{font-family:Georgia,serif;font-size:18px;font-weight:500;line-height:1.15;margin:2px 24px 4px 0;color:#173a34}.mrp-kicker{font-size:8px;text-transform:uppercase;letter-spacing:.1em;color:#9a552e;font-weight:800}.mrp-reach{font-size:10px;color:#66726d;margin-bottom:7px}.mrp-chips{display:flex;gap:4px;flex-wrap:wrap;margin:6px 0 8px}.mrp-chip{font-size:8px;padding:3px 6px;border-radius:999px;background:#eef2ef;color:#46544d;font-weight:700}.mrp-note{font-size:10px;line-height:1.45;margin:7px 0 9px;color:#53615a}.mrp-facts{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin:7px 0}.mrp-facts>div{background:#f5f6f3;border-radius:6px;padding:6px}.mrp-facts span,.mrp-stat-list span{display:block;font-size:7px;text-transform:uppercase;letter-spacing:.07em;color:#87908b;font-weight:700}.mrp-facts b,.mrp-stat-list b{display:block;font-size:10px;color:#26352e;margin-top:1px}.mrp-facts small,.mrp-stat-list small{display:block;font-size:8px;line-height:1.35;color:#7a847f;margin-top:2px}.mrp-live{border-top:1px solid #e6e6df;border-bottom:1px solid #e6e6df;padding:8px 0;margin:8px 0}.mrp-loading{font-size:9px;color:#7b8580;font-style:italic}.mrp-live-grid{display:grid;gap:8px}.mrp-live-grid h4{font-size:9px;text-transform:uppercase;letter-spacing:.07em;color:#476159;margin:0 0 4px}.mrp-stat-list{list-style:none;padding:0;margin:0;display:grid;gap:4px}.mrp-stat-list li{background:#f7f8f5;border-radius:5px;padding:5px 6px}.mrp-actions{display:flex;flex-wrap:wrap;gap:5px;margin-top:8px}.mrp-actions a{font-size:9px;color:#24574d!important;background:#eef5f2!important;border:1px solid #d4e3dd;border-radius:5px;padding:5px 7px;text-decoration:none!important}.mrp-actions .mrp-nav{background:#0d5c63!important;color:#fff!important;border-color:#0d5c63;font-weight:800}.mrp-source,.mrp-caution{font-size:7.5px;line-height:1.4;color:#858e89;margin:7px 0 0}.mrp-caution{background:#fff8e9;border-left:2px solid #d09a4d;padding:5px 6px}.mrp-live-badge{display:inline-block;font-size:8px;text-transform:uppercase;border-radius:999px;padding:3px 6px;margin:3px 0 7px}.mrp-live-badge.yes{background:#e2f1e4;color:#236335}.mrp-live-badge.no{background:#eeeae4;color:#73675a}@media(max-width:900px){.manistee-river-key{left:10px;bottom:48px;width:176px;max-height:178px;overflow:auto}.mi-metrics{grid-template-columns:1fr 1fr}.manistee-rich-popup .leaflet-popup-content-wrapper{max-width:calc(100vw - 34px)}.mrp-card{padding:11px 12px}.mrp-card h3{font-size:17px}}@media(max-width:480px){.manistee-river-key{font-size:10px;width:162px;bottom:44px}.manistee-river-key small{display:none}.mi-metrics{grid-template-columns:1fr}.mrp-facts{grid-template-columns:1fr}.mrp-live-grid{gap:6px}.mrp-note{max-height:70px;overflow:auto}}`;
...
function init(){addStyles();addLegend();observeDetails();observePopups();systemDepth();}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
window.ManisteeDepthTest={hav,nearestGauge,popupLiveHtml};
})();