...
// Mackinac Bridge is a route gateway for travelers arriving from Lower Michigan, not a numbered Circle Tour stop.
const MACKINAC_GATEWAY = Object.freeze({
  coordinates: [-84.72838, 45.81501],
  name: "Mackinac Bridge",
  subtitle: "Lower Michigan gateway · not a Circle Tour stop",
  liveHref: "/mackinac-bridge-live/",
  tollHref: "/mackinac-bridge-tolls/",
});
...
function gatewayFeatures() {
  return {
    ...
    features: [{
      type: "Feature",
      geometry: {type: "Point", coordinates: MACKINAC_GATEWAY.coordinates},
      properties: {name: MACKINAC_GATEWAY.name, subtitle: MACKINAC_GATEWAY.subtitle},
    }],
  };
}
...
function openGatewayPopup({move = true, measure = false} = {}) {
  ...
  if (measure) page.track("map-gateway-open", {gateway: "mackinac-bridge"});
}
...
try {
  ...
  map.on("load", () => {
    ...
    map.addSource("route-gateways", {type: "geojson", data: gatewayFeatures()});
    ...
    map.addLayer({
      id:"gateway-halo",type:"circle",source:"route-gateways",
      paint:{"circle-radius":["interpolate",["linear"],["zoom"],4.5,10,8,13],"circle-color":"#ffffff","circle-stroke-color":"#172d22","circle-stroke-width":2,"circle-opacity":0.96}
    });
    map.addLayer({
      id:"gateway-core",type:"circle",source:"route-gateways",
      paint:{"circle-radius":["interpolate",["linear"],["zoom"],4.5,6,8,8],"circle-color":"#d39a24","circle-stroke-color":"#d39a24","circle-stroke-width":1}
    });
    map.addLayer({
      id:"gateway-label",type:"symbol",source:"route-gateways",
      layout:{"text-field":"Mackinac Bridge · gateway","text-size":11,"text-offset":[0,1.9],"text-anchor":"top","text-allow-overlap":false},
      paint:{"text-color":"#172d22","text-halo-color":"#ffffff","text-halo-width":1.5}
    });
