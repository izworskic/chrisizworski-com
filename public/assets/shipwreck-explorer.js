(()=>{
  ...
  function anchorFor(place){const found=anchors.find(([re])=>re.test(place));return found?{lat:found[1],lng:found[2],label:found[3]}:null;}
  ...
  function matches(r){const n=q.value.trim().toLowerCase();if(n&&!`${r.name} ${r.place} ${r.type}`.toLowerCase().includes(n))return false;if(lake.value&&r.lake!==lake.value)return false;if(eraEl.value&&r.era!==eraEl.value)return false;if(causeEl.value&&r.cause!==causeEl.value)return false;if(accessEl.value&&r.access!==accessEl.value)return false;if(preset==='1913'&&r.year!==1913)return false;if(preset==='superior'&&r.lake!=='Superior')return false;if(preset==='huron'&&r.lake!=='Huron')return false;if(preset==='dive'&&r.access!=='dive')return false;if(preset==='fatal'&&r.deaths<25)return false;return true;}
