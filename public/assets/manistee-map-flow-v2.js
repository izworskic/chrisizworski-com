...
let activeNative=null,activeCard=null,layoutRaf=0;
...
function installStyles(){
  ...
  .manistee-key-toggle-v2{position:absolute;z-index:735;left:12px;bottom:48px;min-height:38px;padding:7px 11px;border:1px solid rgba(31,61,51,.22);border-radius:8px;background:rgba(255,255,255,.97);box-shadow:0 3px 12px rgba(0,0,0,.14);font:700 11px/1 Inter,system-ui,sans-serif;color:#29463d;cursor:pointer}
  #manistee-map .leaflet-popup.manistee-rich-popup{opacity:0!important;pointer-events:none!important}
  .manistee-detached-card{position:absolute;z-index:730;width:min(560px,calc(100% - 32px));max-height:min(56vh,430px);overflow:auto;background:#fff;border:1px solid rgba(32,57,48,.14);border-radius:10px;box-shadow:0 12px 38px rgba(18,35,29,.28);font-family:Inter,system-ui,sans-serif;color:#2a3932;overscroll-behavior:contain}
...
function ensureCard(){
  ...
  close.addEventListener('click',()=>{const x=$('#manistee-map .leaflet-popup.manistee-rich-popup .leaflet-popup-close-button');x?.click();hideCard();});
  card.append(close,body);wrap.appendChild(card);activeCard=card;return card;
}
function hideCard(){if(activeCard)activeCard.hidden=true;activeNative=null;}
function positionCard(native){
  ...
  cancelAnimationFrame(layoutRaf);layoutRaf=requestAnimationFrame(()=>{
    ...
    if(mr.width<=760){card.style.left='10px';card.style.right='10px';card.style.top='auto';card.style.bottom='10px';return;}
...
function syncNative(native){
  ...
  activeNative=native;card.hidden=false;positionCard(native);
}
function scan(){
  setupKey();
  const popups=$$('#manistee-map .leaflet-popup.manistee-rich-popup');
  const native=popups.at(-1)||null;
  if(native)syncNative(native);else if(activeNative)hideCard();
}
function init(){
  installStyles();ensureCard();
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeKey();const x=$('#manistee-map .leaflet-popup.manistee-rich-popup .leaflet-popup-close-button');x?.click();hideCard();}});
  window.addEventListener('resize',()=>activeNative&&positionCard(activeNative));
  scan();
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
window.ManisteeMapFlowV2Test={setupKey,positionCard,syncNative};
})();