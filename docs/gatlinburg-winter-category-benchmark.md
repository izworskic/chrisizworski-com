...
Required changes:
- [ ] Keep the first decision above personalization.
- [ ] Add a compact real-photo visual using a reusable licensed source; never a giant hero.
- [ ] Add plan-at-a-glance: stop count, time span, spend profile, walking load, reservation burden and verification burden.
- [ ] Enrich each itinerary stop with deterministic practical metadata and a short why-now note.
- [ ] Replace the schematic route diagram with an interactive basemap; retain a no-library fallback.
- [ ] Add one “Before you go” checklist that separates checked, recheck and unavailable states.
- [ ] Never state live parking availability unless supplied by an official live source.
- [ ] Never copy weather, closure or operating truth across comparison dates.
- [ ] Never turn cost bands into invented dollar prices.
- [ ] Keep mobile 390 px usable without horizontal page scroll.
- [ ] Preserve all Gatlinburg v1–v3.4 regression tests and add tests for the benchmark-derived layer.
