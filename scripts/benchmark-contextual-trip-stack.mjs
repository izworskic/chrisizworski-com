...
check('AI discovery describes trip network',llms.includes('Contextual Michigan Trip Network')&&llms.includes('fall traffic')&&llms.includes('Manistee River Field Map'),5);
check('Benchmark is in full release gate',pkg.scripts['benchmark:trip-stack']==='node scripts/benchmark-contextual-trip-stack.mjs'&&pkg.scripts['verify:all'].includes('benchmark:trip-stack'),5);
