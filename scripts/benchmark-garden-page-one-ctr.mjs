...
const zoneLedger = (ledger.experiments || []).find((e) => e.id === "2026-08-22-zone-6a-calendar-ctr") || null;
const heirloomLedger = (ledger.experiments || []).find((e) => e.id === "2026-08-22-heirloom-tomatoes-ctr") || null;
check("zone page claims no unbacked freeze", ledger.status === "retired" ? zoneLedger === null : Boolean(zoneLedger?.lastSearchFacingChangeDate));
check("heirloom page claims no unbacked freeze", ledger.status === "retired" ? heirloomLedger === null : Boolean(heirloomLedger?.lastSearchFacingChangeDate));
