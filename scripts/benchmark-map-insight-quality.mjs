...
const checks={
  authenticMedia:{max:20,earned:15,ok:pass(
    boatHtml.includes('Lake_erie_metropark_boat_launch.JPG'),boatHtml.includes('Dwight Burdette'),boatHtml.includes('CC BY 3.0'),provenance.type==='real-photograph',provenance.license==='CC BY 3.0'
  )},
  boatDataKeyedCorrelation:{max:20,earned:20,ok:pass(
    boatApi.includes('PRDBASPublicView/FeatureServer/0'),!boatApi.includes('facilityid IS NOT NULL'),boatApi.includes('globalid'),boatApi.includes('OBJECTID'),boatApi.includes('sourceId(a)'),
    boatJs.includes("const SOURCE_API='/api/boat-launches'"),boatJs.includes('const markerById=new Map()'),boatJs.includes('markerById.set(a.id,m)'),boatJs.includes('data-launch-id'),boatJs.includes("m.on('click',()=>select(a.id,'marker'))"),!boatHtml.includes('id="locdata"'),!boatCode.includes('bestMatch')
  )},
  boatBidirectionalControl:{max:20,earned:20,ok:pass(
    boatJs.includes('function select(id,source='),boatJs.includes('markerById.get(id)'),boatJs.includes('marker.openPopup()'),boatJs.includes('data-popup-card'),boatJs.includes('Show on map'),boatJs.includes('google.com/maps/dir/?api=1&destination='),boatJs.includes('destinationMarker'),boatJs.includes('fitBounds')
  )},
  ...
  trustSearchAccessibility:{max:10,earned:10,ok:pass(
    boatApi.includes('fallback_used: false'),boatJs.includes('No legacy or guessed launch pins are being shown.'),boatApi.includes('String(a.flag || "").trim()'),boatGeo.includes('Destination lookup unavailable'),boatHtml.includes('<link rel="canonical" href="https://chrisizworski.com/michigan-boat-launches/">'),wreckHtml.includes('<link rel="canonical" href="https://chrisizworski.com/great-lakes-shipwrecks/">'),wreckJs.includes('not wreck coordinates'),wreckJs.includes('cited source or agency remains authoritative')
  )}
};
...
const fatalFailures=[];
if(!checks.boatDataKeyedCorrelation.ok)fatalFailures.push('Boat map does not derive from one normalized DNR source record ID.');
if(!checks.boatBidirectionalControl.ok)fatalFailures.push('Boat destination/map ↔ record control is incomplete.');
