...
check("cached-profile","Cross-page personalization reuses the saved JEV persona instead of reclassifying on each page",allIncludes(intel,["profileWithCachedPrimary","cached-jev-profile"])&&hub.includes('mode:"surface"')&&hub.includes("primary_id"),8);
...
check("one-followup","Client asks at most one adaptive follow-up before preserving the profile",allIncludes(hub,["state.adaptiveAsked","next_question","Optional follow-up"]),6);
check("state-continuity","Cross-page CTAs preserve the shared Mackinac trip state",allIncludes(hub,["mackinac-trip-plan-v1","MackinacTripState","plannerHref","rewritePlannerLinks"]),10);
check("surface-focus-ui","Each page can render its visitor-specific focus above generic content",allIncludes(hub,["surface-focus","surface_decision","mackinac_surface_personalized"])&&css.includes(".surface-focus"),8);
check("nav-priority","Shared profile can prioritize destination navigation",hub.includes("markPriorityTabs")&&css.includes(".trip-priority"),4);

check("hub-shell","Primary hub generator includes universal trip intelligence and trip-state client",allIncludes(hubGen,["data-trip-context-work","data-trip-context-actions","mackinac-trip-state.js?v=20260920-live20","mackinac-hub.js?v=20260920-hub2"]),8);
check("intent-shell","Search-intent generator includes universal trip intelligence and trip-state client",allIncludes(intentGen,["profileBox()","data-trip-context-work","mackinac-trip-state.js?v=20260920-live20","mackinac-hub.js?v=20260920-hub2"]),8);
...
check("generated-state","Every generated Mackinac surface loads the shared trip-state client",generated.length===generatedPaths.length&&generated.every(x=>x.includes("mackinac-trip-state.js?v=20260920-live20")),8);
