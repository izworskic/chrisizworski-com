...
for(const [source,target] of files){
  const text=await fetchText(`${rawBase}/${source}?sync=${Date.now()}`);
...
await fs.writeFile('public/national-tools/garden-water/deployment-source.json',JSON.stringify({sourceRepo,sourceRef,sourceCommit:commit.sha,syncedAt:new Date().toISOString()},null,2)+'\n','utf8');
