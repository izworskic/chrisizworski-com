...
const pages=[
  {
    ...
    body:`
      <section class="hub-section"><div class="eyebrow">The shared engine</div><h2>Ask only what changes the trip</h2><p>The My Trip flow now follows the way a visitor actually plans: define the trip shape, solve the mainland approach, account for the people and movement needs, then protect the experiences that matter. Leave-home time is optional. If it is not a real constraint, the planner calculates it backward from the selected ferry instead of asking the visitor to guess.</p><div class="flow-grid"><article><strong>Trip shape</strong><span>Day · overnight · flexible date · still deciding</span></article><article><strong>Getting there</strong><span>Starting point · both ferry ports · optional leave-time constraint</span></article><article><strong>People + movement</strong><span>Party · walking comfort · hills and transitions</span></article><article><strong>What matters</strong><span>Protect two priorities · avoid one failure mode</span></article></div></section>
      <section class="hub-section"><div class="eyebrow">What happens next</div><h2>The planner should remove impossible options before ranking anything</h2><div class="steps"><div class="step"><div><strong>Resolve the mainland approach</strong><p>Your starting city is used to compare travel to Mackinaw City and St. Ignace. Add an earliest leave time only when it is a real constraint; otherwise the planner calculates when to leave for the selected ferry.</p></div></div><div class="step"><div><strong>Keep only reachable ferries</strong><p>Drive time, road buffer and ferry check-in happen before a departure is considered usable.</p></div></div><div class="step"><div><strong>Build a trip shape that fits the visit</strong><p>Arrival day, full Island day, departure day, biking, walking tolerance, meals and fixed event times become real constraints.</p></div></div><div class="step"><div><strong>Rank valid alternatives</strong><p>JEV can resolve preference tradeoffs only after deterministic feasibility has done its work.</p></div></div></div></section>
    ...
    faq:[
      ["How much do I need to know before I start?","Very little. The planner moves through four short stages: trip shape, getting there, people and movement, and what matters. If you do not know a date, origin or trip length yet, it stops at the correct planning boundary instead of inventing precision."],
...
</main><footer><div class="shell">Built by <a href="/">Chris Izworski</a>. Live agencies, operators and businesses remain authoritative for their own schedules, conditions, accessibility, prices and availability.</div></footer><script src="/assets/mackinac-hub.js?v=20260921-human2" defer></script></body></html>`;
}
