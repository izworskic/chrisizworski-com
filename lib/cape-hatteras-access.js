...
function normalizeDirection(direction) {
  const status = String(direction.status_raw||"").toLowerCase();
  const hours = String(direction.hours||"").toLowerCase();
  const notes = String(direction.notes||"").toLowerCase();
  const context = `${status} ${notes}`;
  let state = "unknown";

  // Explicit directional status is authoritative. Hours may clarify a bare
  // "Open" direction, but may never turn explicit pedestrian access into ORV access.
  if (/closed|no public access/.test(status)) state = "closed";
  else if (/pedestrian/.test(status) && !/orv/.test(status)) state = /seasonal/.test(`${status} ${notes}`) ? "seasonal_pedestrian_only" : "pedestrian_only";
  else if (/orv/.test(status) && mileageFrom(context) != null) state = "orv_open_limited";
  else if (/orv/.test(status)) state = "orv_open";
  else if (/\bopen\b/.test(status) && /orv/.test(hours)) state = mileageFrom(context) != null ? "orv_open_limited" : "orv_open";
  else if (/pedestrian/.test(context)) state = /seasonal/.test(context) ? "seasonal_pedestrian_only" : "pedestrian_only";

  return { ...direction, state, mileage: mileageFrom(context) };
}
