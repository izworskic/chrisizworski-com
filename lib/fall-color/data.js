...
async function getWeatherAll() {
  ...
  return arr.map(normalizeWeather);
}

function normalizeWeather(loc) {
    const d = loc?.daily || {};
    const times = d.time || [];
    ...
    const valid = past.filter(Number.isFinite);
    // The adjustment counts cold nights across a fixed 15-day window. Partial
    // history cannot be treated as fewer cold nights (an invented warm signal).
    const summary = valid.length === 15 ? {
      ...
      frostForecast: future.some(Number.isFinite) ? future.some((t) => Number.isFinite(t) && t <= 32) : null,
    } : null;
    const forecast = [];
    for (let i = todayIdx; i < Math.min(times.length, todayIdx + 7); i++) {
      forecast.push({
        date: times[i],
        hi: Number.isFinite(maxs[i]) ? maxs[i] : null,
        lo: Number.isFinite(mins[i]) ? mins[i] : null,
        code: Number.isFinite(codes[i]) ? codes[i] : null,
        pop: Number.isFinite(pops[i]) ? pops[i] : null,
      });
    }
...
module.exports = { getConditions, getWeatherAll, getNdvi, normalizeWeather };
