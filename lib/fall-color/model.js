...
function weatherFeel(forecast) {
  ...
  const codes = next.map((d) => d.code).filter(Number.isFinite);
  ...
  if (!codes.length && !his.length && !los.length) return null;
  let s = !codes.length ? "sky forecast unavailable" : rainy ? "some rain in the next few days" : clear ? "mostly clear skies" : "mixed skies";
...
function snapshotFor(r, cond, anchor) {
  ...
  return {
    ...
    frost: w ? w.frostRecent : null,
    hardFreeze: w ? w.hardFreezeRecent : null,
...
module.exports = { snapshotFor, inSeason, isFallReportSeason, etParts, ndviWeight, weatherFeel };
