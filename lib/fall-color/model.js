...
// The daily writer opens on August 20, three weeks before colour actually turns.
// That is deliberate and it is a search decision, not a content one. Fall colour is a
// freshness-sensitive query class: Google decides who ranks for the season during the
// weeks demand is ramping, which is late August into early September, and a section
// that sits static until September 10 arrives after that decision is made. The
// early-season reports are honest on their own; asked to write while every region
// reads six percent, the model says there is nothing to chase yet and gives the
// timeline instead, which is exactly what an August searcher is asking for.
function inSeason() {
  const { m, d } = etParts();
  if (m === 8) return d >= 20;
