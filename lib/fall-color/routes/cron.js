...
const SYSTEM = [
  ...
  "You are given regional model estimates, their actual source drivers and source dates, weather context, estimated peak windows and canoe runs. Percentages are model outputs, not observed leaf counts. Identify estimates as estimates. Satellite and camera inputs can lag; never describe an old reading as a current observation. Do not imply firsthand visits, specific trees changing color, current river appearance or guaranteed future color. Unknown inputs remain unknown. Use only the provided place names and facts.",
...
module.exports = async (req, res) => {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    // Configuration failure, not a skip. 500 so the cron goes red the day it breaks.
  ...
  const lines = snap.map((s) =>
    "- " + s.name + " (" + s.area + "): model estimate " + s.label + ", about " + s.pct + " percent. " +
    "Source evidence: " + JSON.stringify(s.source) + ". " +
  ...
  const rivers = CANOE.map((c) => {
    ...
    return "- " + c.name + " (" + c.stretch + "): " + c.level + ", " + c.hours +
      ", regional modeled color about " + (s ? s.pct : "?") + " percent.";
  }).join("\n");

  const userMsg = "Date: " + new Date().toLocaleDateString("en-US", { timeZone: "America/Detroit", weekday: "long", month: "long", day: "numeric" }) +
    ".\n\nRegional model estimates, north to south:\n" + lines +
    "\n\nCanoe runs with regional modeled color:\n" + rivers +
  ...
  try {
    ...
    if (!body && aj && aj.error) { res.status(502).json({ error: "anthropic", detail: aj.error }); return; }
  } catch (e) {
    res.status(502).json({ error: "anthropic-fetch", detail: String((e && e.message) || e) });
    return;
  }
  if (!body) { res.status(502).json({ error: "empty-generation" }); return; }

  const obj = annotateReport({ date: dateKey, updated: new Date().toISOString(), body, conditionsUpdated: cond.updated, sourceEvidence: snap.map(s => ({ region: s.id, modeledPercent: s.pct, source: s.source })) });
  try {
