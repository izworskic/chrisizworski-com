...
function scoreBeach({ beach, weather, lakeConditions, waterQuality, hazards = [] }) {
  if (!weather) {
    return {
      ...
      components: null,
      confidence: "low",
      reasons: ["The weather forecast did not load."],
    };
  }
  ...
  const components = {
    ...
    water_temperature: scoreWaterTemperature(lakeConditions?.fresh ? lakeConditions.water_temp_f : null),
    waves: scoreWaves(lakeConditions?.fresh ? lakeConditions.wave_height_ft : null),
  ...
  if (waterQuality?.state === "closure") {
    score = 0;
    reasons.push("Michigan BeachGuard lists an active closure.");
  } else if (waterQuality?.state === "advisory") {
  ...
  if (beach.swimming === false) {
    score = 0;
    reasons.push("This shoreline stop is not a designated swim beach.");
  }

  score = Math.round(clamp(score, 0, 100));
  const recommendation = recommendationForScore(score);
  if (waterQuality?.state === "closure") {
    recommendation.level = "closed";
    recommendation.label = "Beach closure: stay out of the water";
  } else if (waterQuality?.state === "advisory") {
