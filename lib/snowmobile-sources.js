...
function focusTrailReport(body) {
  const text = String(body || "");
  const startMatch = text.match(/(?:Snowmobile\s+)?Trail\s+Report/i);
  const start = startMatch ? Math.max(0, startMatch.index) : 0;
  let excerpt = text.slice(start, start + 3600);
  const stop = excerpt.search(/Popular\s+Trails|Rules\s*&\s*Regulations|Snowmobile\s+Rentals/i);
  if (stop > 400) excerpt = excerpt.slice(0, stop);
  return excerpt.trim();
}
...
function conditionFromText(text) {
  ...
  if (/\bpoor\b|\bbare\b|\bmuddy?\b|\bno good\b|\bnot good\b/.test(t)) return "POOR";
  if (/\bfair\b|\bmarginal\b|\bthin\b|standing water|water (?:holes?|near|on the trail)/.test(t)) return "FAIR";
...
function makeReport({id,name,url,text,authorityWeight=1}) {
  const body = stripHtml(text).slice(0, 9000);
  const excerpt = focusTrailReport(body);
  return {
    id, name, url, sourceType: "local-trail-report", authorityWeight,
    available: true,
    retrievedAt: new Date().toISOString(),
    reportedAt: extractDate(excerpt),
    condition: conditionFromText(excerpt),
    grooming: groomingFromText(excerpt),
...
module.exports = {
  ...
  stripHtml, focusTrailReport, extractDate, conditionFromText, groomingFromText
};
