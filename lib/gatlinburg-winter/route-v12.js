...
async function buildPrepass(rawQuery) {
  ...
  const data = await storage.run(state, () => v10.buildDecision(rawQuery));
...
function personaScore(option, data, policy) {
  ...
  if (policy.persona === "snow") {
    ...
    if (policy.goal === "natural") {
      score += f.hasNps || f.hasScenic ? 14 : 0;
      score -= f.hasTubing ? 8 : 0;
    }
...
async function buildDecision(rawQuery = {}) {
  ...
  data = {
    ...data,
    ...applied,
    benchmarkVersion: "5.0",
    personaEngineVersion: "1.0",
...
module.exports._test = { canonicalPersona, personaPolicy, optionFeatures, personaScore, preferredMatch, rankPersonaOptions, personaReason, personaLens, deferFutureLiveHealth };
