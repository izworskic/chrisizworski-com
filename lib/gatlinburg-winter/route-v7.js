...
const SOURCES = Object.freeze({
  ...
  gatlinburgWebcams: "https://www.gatlinburg.com/plan/webcams/",
...
function stopWhy(data, candidate) {
...
function buildCommitChecks(data, cards) {
  ...
  if (downtown) {
    checks.push({
      ...
      detail: "Visit Gatlinburg links directly to the City of Gatlinburg's live parking information. Check that official city source before entering the downtown core; this planner does not invent stall counts.",
...
function buildLookNow(data, cards) {
  const mountain = cards.some(x => ["mountain", "snow", "scenic", "view", "park"].includes(x.category));
  const downtown = cards.some(x => ["downtown-core", "downtown-north", "skypark", "anakeesta"].includes(x.zone));
  const items = [];
  ...
  if (downtown) items.push({
    ...
    note: "Official Gatlinburg webcam collection, including downtown and attraction views.",
    sourceUrl: SOURCES.gatlinburgWebcams,
    sourceLabel: "Official webcams"
  });

  if (mountain) items.push({
    label: "See the Smokies now",
    note: "NPS current-conditions and webcam context for the park; higher elevations can differ sharply from downtown.",
    sourceUrl: SOURCES.npsConditions,
    sourceLabel: "NPS conditions"
  });

  if ((data?.itinerary || []).some(x => x.id === "winter-magic-walk") || data?.input?.mustLights) items.push({
    label: "Open the lights map",
    note: "Official self-guided Winter Magic map and snowpeople scavenger-hunt information.",
    sourceUrl: SOURCES.winterMagic,
    sourceLabel: "Winter Magic"
  });
  ...
  if (downtown) items.push({
    ...
    note: "Official City of Gatlinburg parking information. Availability belongs to the city source, not this planner.",
    sourceUrl: SOURCES.cityParking,
    sourceLabel: "City parking"
  });
...
async function buildDecision(rawQuery = {}) {
  ...
  const commitChecks = buildCommitChecks(data, cards);
  return {
    ...data,
    version: "3.5",
    ...
    lookNow: buildLookNow(data, cards),
    diagnostics: {
      ...(data.diagnostics || {}),
