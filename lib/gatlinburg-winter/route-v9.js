...
async function buildDecision(rawQuery={}){
  const data=await v8.buildDecision(rawQuery),lens=await chooseLens(data),writer=await writeCopy(data,lens),applied=applyCopy(data,writer);
...
module.exports._test={safe,fallbackLens,facts,fallbackCopy,parseJson,cleanCopy,applyCopy};
