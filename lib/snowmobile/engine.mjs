...
export function closureForSegment(seg,closures=[]){
  ...
  return closures.find(c=>{
    ...
    // digit-prefixed ("LP 4" vs "LP 47" vs "LP 487" vs "LP 404"), and a
    // substring check here previously made one real "LP 4" closure falsely
    // also close every trail whose number happens to start with "4" -
    // confirmed live against the DNR closures layer, where a single LP 4
    // closure was silently flagging LP 47, LP 487, LP 482 and LP 404 as
    // closed too. DNRTrail is also always the literal placeholder string
    // "DNR Trail" on every closure record (never a real per-trail id), so
    // it is explicitly excluded from matching rather than left to
    // coincidentally never equal a real segment id.
