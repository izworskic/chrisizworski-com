...
export function scoreSegment(seg,{clubReport,weather,season=true,verifiedClosure=null}={}){
  ...
  if(verifiedClosure)return {score:0,band:'CLOSED',reasons:[verifiedClosure.properties?.PublicComm||'Official DNR temporary-closure layer matches this trail.'],veto:true,legalState:'CLOSED',closure:verifiedClosure.properties};
  ...
  return {score,band:score>=85?'EXCELLENT':score>=72?'GOOD':score>=58?'FAIR':score>=40?'MARGINAL':'POOR',reasons,legalState:'NO_MATCH_IN_CURRENT_CLOSURE_LAYER'};
}
...
export function rankRideWindows(weather={},season=true){
  ...
  for(const [location,data] of Object.entries(weather||{})){
    for(const p of (data?.periods||[])){
      ...
      const w=windMph(p.windSpeed); if(Number.isFinite(w))row.winds.push(w);
      row.texts.push(`${p.shortForecast||''} ${p.detailedForecast||''}`);
      byStart.set(key,row);
    }
  }
  const windows=[...byStart.values()].map(w=>{
    const maxTempF=w.temperatures.length?Math.max(...w.temperatures):null;
    const minTempF=w.temperatures.length?Math.min(...w.temperatures):null;
    const maxWindMph=w.winds.length?Math.max(...w.winds):null;
    const text=w.texts.join(' ').toLowerCase();
    ...
    else if(/snow/.test(text)){score+=3;reasons.push('snow in forecast');}
    ...
    if(w.isDaytime)score+=3;
    return {...w,maxTempF,minTempF,maxWindMph,score:clamp(Math.round(score)),reasons};
  }).sort((a,b)=>b.score-a.score||new Date(a.startTime)-new Date(b.startTime));
  return {best:windows[0]||null,windows,boundary:'This ranks forecast periods only. It does not upgrade trail condition, legal status, grooming freshness or trail base.'};
}
