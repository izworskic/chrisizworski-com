...
function normalizeOpenTrailFeature(feature){
  ...
  return {...feature,properties:{
    ...
    Trail_Netw:p.TrailNetwork||p.TrailNamePrimary||p.SnowmobileName||null,
...
async function fetchDnrOpenCorridor(){
  const p=corridorParams("OBJECTID,GlobalID,DNRTrail,TrailApprovalStatus,OpenClosedStatusSnowmobile,Snowmobile,SnowmobileName,TrailNamePrimary,TrailGroomType,TrailGrooming,SurfaceType,TrailOnRoad,SegmentLengthMiles,PublicComments,TrailNetwork,County,SpecialRestrictionType,TrailOwnership,TrailAdministrator,last_edited_date");
  const r=await fetch(`${DNR_OPEN}?${p}`);
  if(!r.ok)throw new Error(`DNR open-data ArcGIS ${r.status}`);
  const json=await r.json();
  if(!Array.isArray(json?.features)||!json.features.length)throw new Error('DNR open-data corridor returned no features');
