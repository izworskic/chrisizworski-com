...
export async function fetchDnrCorridor(){
  const params=new URLSearchParams({
    ...
    outFields:"FID,Unique_ID,Status,Snowmobile,Snowmobi_1,Groom_Spon,Trail_Netw,Owner,Administra,Surface,Seasonal_R,Seasonal_1,Special_De,Special__1,On_Road,ROW,ROW_Commen,Miles,Comments,EditDate",
    returnGeometry:"true",outSR:"4326",f:"geojson"
  });
  const r=await fetch(`${DNR}?${params}`); if(!r.ok) throw new Error(`DNR ArcGIS ${r.status}`); return r.json();
}
async function nwsPoint(lat,lon){
  const p=await fetch(`https://api.weather.gov/points/${lat},${lon}`,{headers:{'user-agent':'chrisizworski.com snowmobile conditions'}}); if(!p.ok)throw new Error(`NWS points ${p.status}`);
  const pj=await p.json(); const f=await fetch(pj.properties.forecast,{headers:{'user-agent':'chrisizworski.com snowmobile conditions'}}); if(!f.ok)throw new Error(`NWS forecast ${f.status}`);
  return f.json();
}
function weatherSummary(json){
  ...
  return {maxTempF:temps.length?Math.max(...temps):null,minTempF:temps.length?Math.min(...temps):null,snowIn,rainIn,periods:next.map(p=>({name:p.name,temperature:p.temperature,windSpeed:p.windSpeed,shortForecast:p.shortForecast,startTime:p.startTime,detailedForecast:p.detailedForecast}))};
}
export async function fetchWeather(){
  const [grayling,gaylord]=await Promise.allSettled([nwsPoint(44.6614,-84.7148),nwsPoint(45.0275,-84.6748)]);
  return {
    grayling:grayling.status==='fulfilled'?weatherSummary(grayling.value):{error:String(grayling.reason)},
    gaylord:gaylord.status==='fulfilled'?weatherSummary(gaylord.value):{error:String(gaylord.reason)}
  };
}
