...
module.exports = async function oldSowRoute(req, res) {
  ...
  try {
    ...
    const futureWindows = (product.windows || [])
      .filter(w => new Date(w.end).getTime() >= now.getTime());
    const nearTermCutoff = now.getTime() + 36 * 3600000;
    const nearTermWindows = futureWindows
      .filter(w => new Date(w.start).getTime() <= nearTermCutoff)
      .slice(0, 5);
    const candidateWindows = nearTermWindows.length >= 2
      ? nearTermWindows
      : futureWindows.slice(0, 5);
