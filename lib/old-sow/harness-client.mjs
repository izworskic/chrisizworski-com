...
export function viewingWindowDecisionPayload(windows, context = {}) {
  ...
  return {
    action: 'decide',
    task: 'Choose the single best practical land-viewing window for a general visitor trying to see Old Sow from Deer Island Point. The candidate windows are already valid NOAA-derived flood-current windows. Rank only among them.',
    options,
    context: {
      viewpoint: 'Deer Island Point Park, New Brunswick',
      visitor_goal: 'Maximize the practical chance of seeing visibly energetic Old Sow conditions without asking a visitor to wait unnecessarily long.',
    ...
    constraints: [
      ...
      'Do not change, invent, interpolate, or reinterpret any candidate time, tide, current, daylight, or weather value.',
      'Prefer daylight over darkness for a general land visitor unless no reasonable daylight candidate is available soon.',
      'Prefer a strong nearby predicted flood current, but do not treat current percentile as whirlpool probability.',
      'Prefer a good near-term window within roughly 24 hours over a slightly stronger window many days away.',
...
export async function recommendViewingWindowWithHarness(windows, context = {}) {
