...
async function chooseSpatialWithJev(candidates, context){
  ...
  return decideClosedSet({...payload,fallbackChoice:top[0].id,minConfidence:.52,timeoutMs:HARNESS_TIMEOUT_MS,gateReason:"JEV spatial output did not pass closed-set confidence gates",unavailableReason:"Shared JEV spatial authentication unavailable"});
}
...
async function chooseMultiDayWithJev(candidates, context){
  ...
  return decideClosedSet({...payload,fallbackChoice:top[0].id,minConfidence:.52,timeoutMs:HARNESS_TIMEOUT_MS,gateReason:"JEV multi-day output did not pass closed-set confidence gates",unavailableReason:"Shared JEV multi-day authentication unavailable"});
}
...
async function chooseWebcamWithJev(cameras, context) {
  ...
  return decideClosedSet({...payload,fallbackChoice:top[0].id,minConfidence:.52,timeoutMs:HARNESS_TIMEOUT_MS,gateReason:"JEV webcam output did not pass closed-set confidence gates",unavailableReason:"Shared JEV webcam authentication unavailable"});
}
...
async function buildPayload(req) {
  ...
  const cachedVisitorPrimary=safeText(req?.query?.visitor_primary||"",80);
  const visitorBase=cachedVisitorPrimary
    ? intelligence.profileWithCachedPrimary(visitorAnswers,cachedVisitorPrimary)
    : intelligence.deterministicProfile(visitorAnswers);
  const visitorIntelligence=tuning.applyVisitor(visitorBase,rawTunings);
