...
const WEBCAM_CATALOG = Object.freeze([
  Object.freeze({
    ...
    view:"Main Street looking west",source_url:"https://www.chippewahotel.com/web-cams/",embed_url:null,
  ...
  Object.freeze({
    ...
    view:"Main Street from the roof near the ferry docks",source_url:"https://www.hornsbar.com/webcamlarge/",embed_url:null,
  ...
  Object.freeze({
    ...
    view:"Historic Market Street toward Fort Mackinac",source_url:"https://www.mackinacislandnews.com/view-fort-mackinac-live",embed_url:null,
  ...
  Object.freeze({
    ...
    view:"Windermere Point, ferry traffic and Round Island",source_url:"https://www.windermerehotel.com/webcam.html",embed_url:null,
...
function webcamCandidates(ctx, likelyPlan=null) {
  ...
  return WEBCAM_CATALOG.map(cam=>{
    ...
    return {...cam,fit_score:round(clamp(score,0,100)),reason:reasons.slice(0,2).join(" · ")||cam.default_reason};
...
async function chooseWebcamWithJev(cameras, context) {
  ...
  const payload={
    ...
    evidence:[{id:"camera-catalog",source:"Mackinac webcam registry",text:safeText(JSON.stringify(top.map(c=>({id:c.id,score:c.fit_score,tags:c.tags,view:c.view}))),3200)}]
  };
  const controller=new AbortController(); const timer=setTimeout(()=>controller.abort(),HARNESS_TIMEOUT_MS);
  try{
    const res=await fetch(HARNESS_URL,{method:"POST",redirect:"error",signal:controller.signal,headers:{authorization:`Bearer ${token}`,"content-type":"application/json",accept:"application/json"},body:JSON.stringify(payload)});
