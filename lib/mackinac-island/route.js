...
async function chooseSpatialWithJev(candidates, context){
  ...
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),HARNESS_TIMEOUT_MS);
  try{
    const res=await fetch(HARNESS_URL,{method:"POST",redirect:"error",signal:controller.signal,headers:{authorization:`Bearer ${token}`,"content-type":"application/json",accept:"application/json"},body:JSON.stringify(payload)});
    const data=await res.json().catch(()=>null);if(!res.ok||!data?.result)throw new Error(`Harness HTTP ${res.status}`);
    const judged=data.result.choice||{},id=judged.choice,conf=Number(judged.confidence)||0,inj=Number(data.result.injection_dependency);
    if(!top.some(c=>c.id===id)||conf<.52||(Number.isFinite(inj)&&inj>=.45))return{mode:"deterministic",choiceId:top[0].id,confidence:conf,reason:"JEV spatial output did not pass closed-set confidence gates"};
    return{mode:"shared-harness-jev",choiceId:id,confidence:conf,model:data.result.model||"jev-latest",reason:null};
  }catch(e){return{mode:"deterministic",choiceId:top[0].id,confidence:0,reason:`JEV spatial selector unavailable: ${safeText(e.message,180)}`};}
  finally{clearTimeout(timer);}
}
...
async function chooseMultiDayWithJev(candidates, context){
  ...
  try{
    ...
    const data=await res.json().catch(()=>null);if(!res.ok||!data?.result)throw new Error(`Harness HTTP ${res.status}`);
    const judged=data.result.choice||{},id=judged.choice,conf=Number(judged.confidence)||0,inj=Number(data.result.injection_dependency);
    if(!top.some(c=>c.id===id)||conf<.52||(Number.isFinite(inj)&&inj>=.45))return{mode:"deterministic",choiceId:top[0].id,confidence:conf,reason:"JEV multi-day output did not pass closed-set confidence gates"};
    return{mode:"shared-harness-jev",choiceId:id,confidence:conf,model:data.result.model||"jev-latest",reason:null};
  }catch(e){return{mode:"deterministic",choiceId:top[0].id,confidence:0,reason:`JEV multi-day selector unavailable: ${safeText(e.message,180)}`};}
  finally{clearTimeout(timer);}
}
