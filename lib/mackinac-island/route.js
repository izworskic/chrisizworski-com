...
function itineraryFor(plan, ctx) {
  ...
  if (profile.mobility === "limited") {
    ...
    if (carriageRequested) add(60,"Horse-drawn carriage / taxi orientation","Make the horse-drawn experience a real part of the plan instead of treating it as optional filler.","carriage");
  ...
  if (Number.isFinite(profile.event_start_minutes)) {
    ...
    if (eventArrival<=profile.event_start_minutes && eventArrival<=hardEnd-60) {
      t=eventArrival;
  ...
  if (fort && (firstVisit || wants("history") || must("fort"))) {
    ...
    if (t+fortMinutes <= close && t+fortMinutes <= hardEnd) {
      add(fortMinutes,"Fort Mackinac",profile.mobility==="limited"?"Use accessible transport as needed; the fort sits above downtown and the approach is steep. Allow time for access rather than rushing.":"Allow about 90–110 minutes. Downtown to the fort is roughly 0.4 mile and uphill.","fort");
    }
  }
...
async function buildPayload(req) {
  ...
  function buildDayState() {
    dateMinute=planningMode==="today"?lp.minutes:7*60;
    ...
    ctx={date:targetDate,personas,origin,profile,hourly,marine,attractions,events,sunset,sunrise,sameDay:planningMode==="today",nowMinutes:lp.minutes};
    candidates=planCandidates(records,ctx);
  }
  buildDayState();
  if(!candidates.length && planningMode==="today"){
