...
function profileFromQuery(query = {}, basePersonas = ["day-trip"], origin = "lower") {
  ...
  const dynamicPreset = dynamicLabel && dynamicPort && Number.isFinite(dynamicDrive) && dynamicDrive >= 0 && dynamicDrive <= 1200
    ? { label:dynamicLabel, preferred_port:dynamicPort, drive_minutes:dynamicDrive, confidence:"OpenStreetMap/OSRM planning estimate", source_url:"https://www.openstreetmap.org/copyright" }
    : null;
  ...
  return {
    ...
    origin_city:originCity, origin_name:preset?.label||null, origin_preset:preset, desired_return_minutes:Number.isFinite(desiredReturn)?desiredReturn:null,
...
async function buildPayload(req) {
  ...
  for (const port of ["Mackinaw City","St. Ignace"]) {
    ...
    const originLead=planningMode==="today" && profile.origin_preset && port===profile.origin_preset.preferred_port
      ? profile.origin_preset.drive_minutes+15
      : 0;
    const validNow=out.filter(r=>planningMode!=="today" || r.departure_minutes>=lp.minutes+originLead+r.checkin_buffer_minutes);
    ...
    portSummary[port]={
      ...
      last_return:back[back.length-1]?.departure_time||null,next_departure:validNow[0]||null,next_departure_origin_adjusted:Boolean(originLead),best_departure:bestCandidate?.outbound||null,
      departures:out
    };
  }
  ...
  const primaryReason=plan
    ? plan.usable_island_minutes==null
      ? `Overnight plan with a ${scoreLabel(plan.components.activity).toLowerCase()} activity window and no artificial last-ferry pressure.`
