...
function planCandidates(records, ctx) {
  ...
  const requestedLeave=Number(ctx.profile?.departure_minutes);
  const currentFloor = Number.isFinite(requestedLeave) ? requestedLeave : (ctx.sameDay ? ctx.nowMinutes : 0);
  const candidates=[];
...
function itineraryFor(plan, ctx) {
  ...
  const requestedLeave=Number(profile.departure_minutes);
...
function decisionReasons(plan, ctx) {
  ...
  if (Number.isFinite(ctx.profile?.departure_minutes)) why.push(`Starts from the entered ${clock(ctx.profile.departure_minutes)} leave-home time; earlier ferries that cannot be reached are excluded`);
...
async function buildPayload(req) {
  ...
  if(Number.isFinite(profile.departure_minutes) && profile.departure_minutes < lp.minutes){
    ...
    if(tomorrowRecords.length){targetDate=tomorrow;records=tomorrowRecords;planningMode="tomorrow";planningReason=`Your entered ${clock(profile.departure_minutes)} leave-home time has already passed today, so this personalized plan uses tomorrow.`;}
  } else if(!todayStillPossible){
    const tomorrow=addLocalDays(lp.date,1); const tomorrowRecords=publishedFerries(tomorrow,verifyArnold,verifySheplers);
    if(tomorrowRecords.length){targetDate=tomorrow;records=tomorrowRecords;planningMode="tomorrow";planningReason="Today’s useful day-trip window has closed, so this plan uses tomorrow.";}
  }

  let dateMinute,attractions,events,weather,crowd,sunrise,sunset,ctx,candidates;
  function buildDayState() {
    dateMinute=planningMode==="today"?lp.minutes:(Number.isFinite(profile.departure_minutes)?profile.departure_minutes:7*60);
