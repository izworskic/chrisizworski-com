...
function profileFromQuery(query = {}, basePersonas = ["day-trip"], origin = "lower") {
  ...
  const trip = query.trip === "overnight" || basePersonas.includes("overnight") ? "overnight" : "day-trip";
  ...
  const dinner = ["none","casual","sit-down"].includes(String(query.dinner)) ? String(query.dinner) : "casual";
  ...
  const personas = [...personaSet].filter(x=>PERSONA_BASE[x]).slice(0,5);
  return {
