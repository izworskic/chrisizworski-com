...
async function buildPayload(req) {
  ...
  const verifyArnold=arnoldR.status==="fulfilled" && /SEPT 7|Sept 7|SUMMER SCHEDULE/i.test(arnoldR.value);
  const verifySheplers=sheplersR.status==="fulfilled";
  ...
  const confidenceParts=[hourly.length?1:0,marine.available?1:0,verifyArnold?1:0,verifySheplers?1:0,hoursR.status==="fulfilled"?1:0];
  ...
  const itinerarySummary=plan?`${plan.outbound.departure_time} ${plan.outbound.origin_port} ferry · about ${plan.outbound.arrival_time} island arrival · ${plan.return?.departure_time||"overnight"} recommended return.`:"No complete itinerary can be built from the verified schedule window.";
  ...
  const ferryFreshnessLabel=verifyArnold&&verifySheplers?"Both operator schedule pages verified this request":verifyArnold||verifySheplers?"One operator verified; one using published 2026 schedule":"Published 2026 schedules; live source recheck degraded";
  ...
  return {
    ...
    sources:{
      ...
      arnold:sourceState(arnoldR,"Arnold Transit Company",SOURCE_URLS.arnold),
      sheplers:sourceState(sheplersR,"Shepler's Mackinac Island Ferry",SOURCE_URLS.sheplers),
    ...
    degraded,operational:{degraded,confidence,data_boundary:"Ferry times are normalized from published 2026 schedules and rechecked against operator pages when reachable. Weather/marine observations are live source data. Crowd pressure is modeled, not observed. JEV may only choose among deterministic feasible plans and cannot create facts."}
  };
}
...
module.exports._test={localParts,addLocalDays,parseClock,clock,arnoldSchedule,sheplersSchedule,mergeWeights,solarMinutes,crowdRead,attractionState,planCandidates,scoreLabel};
