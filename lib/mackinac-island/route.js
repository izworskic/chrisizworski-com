...
function profileFromQuery(query = {}, basePersonas = ["day-trip"], origin = "lower") {
  ...
  const preset = originCity ? ORIGIN_PRESETS[originCity] : null;
  ...
  return {
    trip, adults, children, bikes, pace, mobility, dinner, interests, must_do:mustDo,
    origin_city:originCity, origin_preset:preset, desired_return_minutes:Number.isFinite(desiredReturn)?desiredReturn:null,
...
function planCandidates(records, ctx) {
  ...
  for (const out of outs) {
    const originTravel = ctx.sameDay && ctx.profile?.origin_preset && out.origin_port === ctx.profile.origin_preset.preferred_port
      ? ctx.profile.origin_preset.drive_minutes + 15
      : 0;
    if (out.departure_minutes < currentFloor + originTravel + out.checkin_buffer_minutes) continue;
    ...
    if (Number.isFinite(profile.desired_return_minutes) && ret.departure_minutes > profile.desired_return_minutes) score -= Math.min(24,(ret.departure_minutes-profile.desired_return_minutes)*.4);
...
function itineraryFor(plan, ctx) {
  ...
  if (preset && plan.outbound.origin_port === preset.preferred_port) {
    const leave = start - plan.outbound.checkin_buffer_minutes - preset.drive_minutes - 15;
    items.push({minute:leave,label:`Leave ${preset.label}`,detail:`Planning drive estimate: about ${Math.round(preset.drive_minutes/60*10)/10} hr to ${preset.preferred_port}, plus a 15-minute road buffer. This is not live traffic.`,stop_id:"mainland-drive"});
  }
  items.push(
    { minute:start-plan.outbound.checkin_buffer_minutes,label:`Arrive ${plan.outbound.origin_port} dock`,detail:`Allow ${plan.outbound.checkin_buffer_minutes} minutes for parking/check-in.`,stop_id:"mainland-dock" },
  ...
  function add(duration,label,detail,stop_id,preferredMinute=null) {
  ...
  if (profile.mobility === "limited") {
    add(60,"Horse-drawn taxi / carriage orientation","Use horse-drawn transport to reduce steep walking. Mackinac State Historic Parks notes steep grades and uneven surfaces; wheelchair-accessible taxi vehicles are available by advance request.","carriage");
  } else if (ctx.sameDay && Number(ctx.nowMinutes) >= 12*60 && !profile.interests.length && !profile.must_do.length && profile.bikes === "none") {
    add(75,"Downtown + waterfront short-visit loop","For a late arrival, keep the day compact: Main Street, Marquette Park and the harbor rather than sprinting inland. Downtown is only a few blocks long.","downtown");
  } else if (ctx.personas.includes("biking") || profile.bikes !== "none" || must("m185")) {
    const pickup=profile.bikes === "rent" ? 20 : 5;
    if (pickup>5) add(pickup,"Pick up rental bikes","Keep rental time inside the plan instead of pretending the ride starts immediately.","downtown");
    add(profile.pace==="active"?95:profile.children>0?120:110,"Ride M-185 shoreline loop","About 8.2 miles. The planner puts the loop early when wind/rain and crowd pressure are usually more favorable.","m185");
  } else if (firstVisit && profile.pace !== "easy") {
    add(45,"Downtown + Marquette Park orientation","Start with the harbor, Main Street and Marquette Park before climbing inland.","downtown");
  }

  if (Number.isFinite(profile.event_start_minutes)) {
    const eventArrival=Math.max(t,profile.event_start_minutes-35);
    if (eventArrival<=hardEnd-60) {
      t=eventArrival;
      add(90,"Event block","Arrive roughly 30–35 minutes before the event start so the ferry is not the event check-in plan.","downtown");
    }
  }

  add(profile.children>0?65:55,"Lunch / real break",profile.children>0?"A real meal and restroom break. The family plan deliberately leaves slack.":"Leave enough time to sit down or grab food without turning the rest of the day into a sprint.","downtown");
  ...
  if (fort && (firstVisit || wants("history") || must("fort"))) {
    ...
    if (t < close-70 && t < hardEnd-70) {
      add(profile.pace==="easy"?95:110,"Fort Mackinac",profile.mobility==="limited"?"Use accessible transport as needed; the fort sits above downtown and the approach is steep. Allow time for access rather than rushing.":"Allow about 90–110 minutes. Downtown to the fort is roughly 0.4 mile and uphill.","fort");
    }
  }

  if ((must("arch-rock") || wants("scenery")) && profile.mobility !== "limited") {
    add(profile.pace==="active"?65:80,"Arch Rock / east bluff","Allow for the climb and viewpoint time. Skip this if the return buffer becomes tight.","arch-rock");
  }

  if (profile.children>0 || profile.pace==="easy" || profile.mobility==="limited") {
    add(45,"Easy flex block","Fudge, waterfront, Marquette Park, shopping or a carriage option. This block exists so the trip can absorb fatigue and lines.","downtown");
  } else if (wants("shopping") || wants("food") || firstVisit || must("downtown")) {
    add(45,"Downtown + fudge","Keep some unscheduled Main Street time instead of filling every minute.","downtown");
  }
  ...
  function movementFor(i) {
    if (i.stop_id === "mainland-drive" && preset) return `Road trip → ${preset.preferred_port} · about ${Math.round(preset.drive_minutes/60*10)/10} hr planning estimate + 15 min road buffer`;
...
function decisionReasons(plan, ctx) {
  ...
  const why=[`Preserves ${plan.usable_island_minutes==null?"an overnight stay":Math.round(plan.usable_island_minutes/60*10)/10+" usable island hours"}`,"Keeps a later ferry behind the recommended return when the schedule allows",ctx.personas.includes("biking")?"Prioritizes the stronger riding window":"Balances arrival time, weather and crowd pressure"];
  if (ctx.profile?.origin_preset) why.push(`Uses ${ctx.profile.origin_preset.preferred_port}, the practical port from ${ctx.profile.origin_preset.label}`);
  if (Number.isFinite(ctx.profile?.desired_return_minutes)) why.push(`Targets a return no later than about ${clock(ctx.profile.desired_return_minutes)}`);
...
async function buildPayload(req) {
  ...
  const ctx={date:targetDate,personas,origin,profile,hourly,marine,attractions,events,sunset,sunrise,sameDay:planningMode==="today",nowMinutes:lp.minutes};
  ...
  const itinerarySummary=plan?`${leaveItem?leaveItem.time+" leave "+profile.origin_preset.label+" · ":""}${plan.outbound.departure_time} ${plan.outbound.origin_port} ferry · about ${plan.outbound.arrival_time} island arrival · ${plan.return?.departure_time||"overnight"} recommended return.`:"No complete itinerary can be built from the published schedule window.";
  ...
  return {
    ...
    itinerary,itinerary_summary:itinerarySummary,itinerary_reason:itineraryReason,leave_home:leaveItem?{time:leaveItem.time,label:leaveItem.label,detail:leaveItem.detail}:null,
    ...
    planning_references:{accessibility:{name:"Mackinac State Historic Parks accessibility",url:SOURCE_URLS.accessibility},drive_times:profile.origin_preset?{label:profile.origin_preset.label,minutes:profile.origin_preset.drive_minutes,confidence:profile.origin_preset.confidence,url:profile.origin_preset.source_url,note:"Planning estimate only; not live traffic."}:null},
    degraded,operational:{degraded,confidence,data_boundary:"Ferry times are normalized from official published 2026 schedules. Arnold's current timetable signature is validated against its official page when reachable. Shepler's timetable is a captured official 2026 schedule unless a future adapter can validate its linked timetable directly. Weather/marine observations are live source data. Crowd pressure is modeled, not observed. Origin drive times are planning estimates, never live traffic. JEV may only choose among deterministic feasible plans and cannot create facts."}
  };
}
...
module.exports._test={localParts,addLocalDays,parseClock,clock,arnoldSchedule,sheplersSchedule,mergeWeights,profileFromQuery,preferredPort,solarMinutes,crowdRead,attractionState,planCandidates,itineraryFor,scoreLabel,arnoldSchedulePageVerified,sheplersScheduleSourceReachable,ORIGIN_PRESETS};
