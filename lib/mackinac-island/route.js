...
function planCandidates(records, ctx) {
  ...
  for (const out of outs) {
    ...
    const originTravel = ctx.sameDay && Number.isFinite(driveMinutes) ? driveMinutes + 15 : 0;
    ...
    candidates.push({ id, score, outbound:out, return:retChoice?.row||null, usable_island_minutes:ctx.personas.includes("overnight")?null:stay, mainland_drive_minutes:Number.isFinite(driveMinutes)?driveMinutes:null, components, deterministic_rank_score:score });
  }
...
function itineraryFor(plan, ctx) {
  ...
  if (preset && Number.isFinite(selectedDriveMinutes)) {
    const leave = start - plan.outbound.checkin_buffer_minutes - selectedDriveMinutes - 15;
    items.push({minute:leave,label:`Leave ${preset.label}`,detail:`Planning drive estimate: about ${Math.round(selectedDriveMinutes/60*10)/10} hr to ${plan.outbound.origin_port}, plus a 15-minute road buffer. This is not live traffic.`,stop_id:"mainland-drive"});
  }
  items.push(
  ...
  function movementFor(i) {
    if (i.stop_id === "mainland-drive" && preset && Number.isFinite(selectedDriveMinutes)) return `Road trip → ${plan.outbound.origin_port} · about ${Math.round(selectedDriveMinutes/60*10)/10} hr planning estimate + 15 min road buffer`;
...
async function buildPayload(req) {
  ...
  for (const port of ["Mackinaw City","St. Ignace"]) {
    ...
    const originLead=planningMode==="today" && Number.isFinite(portDriveMinutes) ? portDriveMinutes+15 : 0;
    const validNow=out.filter(r=>planningMode!=="today" || (!profile.origin_preset || Number.isFinite(portDriveMinutes)) && r.departure_minutes>=lp.minutes+originLead+r.checkin_buffer_minutes);
...
module.exports._test={localParts,addLocalDays,parseClock,clock,arnoldSchedule,sheplersSchedule,mergeWeights,profileFromQuery,preferredPort,driveMinutesForPort,accessScore,solarMinutes,crowdRead,attractionState,planCandidates,itineraryFor,scoreLabel,arnoldSchedulePageVerified,sheplersScheduleSourceReachable,ORIGIN_PRESETS};
