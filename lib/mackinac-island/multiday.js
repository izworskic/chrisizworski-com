...
function intent(profile={},visitor={}){
  const v=visitor.vector||{},interests=new Set(profile.interests||[]),must=new Set(profile.must_do||[]),tunings=new Set(profile.tunings||[]);
  return {
    bike:(profile.bikes&&profile.bikes!=="none")||interests.has("biking")||must.has("m185")||visitor.answers?.trip_vision?.includes("biking"),
    history:v.history>=.58||interests.has("history")||must.has("fort")||tunings.has("history"),
    scenery:v.photography>=.62||interests.has("scenery")||interests.has("photography")||must.has("arch-rock")||must.has("sunset")||tunings.has("outdoors"),
    food:v.food>=.7||interests.has("food")||tunings.has("better-dinner"),
    quiet:v.crowd_avoidance>=.72||v.pace<=.35||tunings.has("relaxed")||tunings.has("less-downtown"),
