...
const PROJECT = process.env.VERCEL_PROJECT_ID || "chrisizworski-com";
const TEAM = process.env.VERCEL_TEAM_ID || "izworski-gmailcoms-projects";
...
async function oidcToken(){
  ...
  if(process.env.VERCEL_OIDC_TOKEN) return {token:String(process.env.VERCEL_OIDC_TOKEN),source:"env-oidc"};
  try{
    oidcModulePromise ||= import("@vercel/oidc");
    const mod=await oidcModulePromise;
    const token=await mod.getVercelOidcToken({project:PROJECT,team:TEAM,expirationBufferMs:60_000});
