...
async function harnessPick({task,options,context,constraints,evidence=[]}){
  ...
  if(!ids.length)return{choice:null,confidence:0,mode:"deterministic",reason:"No candidates"};
  const token=process.env.HARNESS_ACCESS_KEY||process.env.VERCEL_OIDC_TOKEN||"";
  if(!token)return{choice:ids[0],confidence:0,mode:"deterministic",reason:"Shared JEV credential unavailable"};
  ...
  try{
    ...
    const judged=data.result.choice||{};const choice=judged.choice;const confidence=Number(judged.confidence)||0;const inj=Number(data.result.injection_dependency);
    if(!ids.includes(choice)||confidence<.52||(Number.isFinite(inj)&&inj>=.45))return{choice:ids[0],confidence,mode:"deterministic",reason:"JEV output did not pass closed-set gates"};
    return{choice,confidence,mode:"shared-harness-jev",model:data.result.model||"jev-latest",reason:null};
  }catch(e){return{choice:ids[0],confidence:0,mode:"deterministic",reason:`JEV unavailable: ${safe(e.message,160)}`};}
