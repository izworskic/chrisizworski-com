...
const PROFILE_VERSION = "mackinac-intelligence-v1";
...
function adaptiveCandidates(raw){
  ...
  // No cap here: truncating in code order before ranking could drop the most useful
  // question. deterministicProfile ranks everything eligible, then keeps the top four.
...
function deterministicProfile(raw){
  ...
  const eligible=adaptiveCandidates(answers);
  // Base questions are always asked in order. Once they are answered, the one follow-up
  // is the question whose answer would move this visitor's trip the most, and none at
  // all if no candidate would visibly change anything. See adaptive-impact.js.
  const adaptiveImpact=eligible.length&&QUESTION_LIBRARY[eligible[0]]?.stage==="adaptive"
    ? require("./adaptive-impact").rankByImpact(module.exports,answers,eligible).slice(0,4)
    : eligible.map(id=>({id,impact:null}));
  const adaptive=adaptiveImpact.map(x=>x.id);
  ...
  return {
    ...
    adaptive_impact:adaptiveImpact,
...
async function harnessPick({task,options,context,constraints,evidence=[]}){
  ...
  if(!ids.length)return{choice:null,confidence:0,mode:"deterministic",reason:"No candidates",auth:"none"};
  const result=await harness.decideClosedSet({
  ...
  const [archPick,qPick]=await Promise.all([
    ...
    questionIds.length?harnessPick({
      ...
      constraints:[
        ...
        "Each candidate carries a measured impact: the average share of this visitor's trip that changes across that question's possible answers. Prefer higher measured impact."
      ],
      evidence:[{id:"measured-follow-up-impact",source:"counterfactual run of the Mackinac personalization engines",text:safe(JSON.stringify(fallback.adaptive_impact),1400)}]
...
module.exports={
  ...
  normalizeAnswers,vectorFromAnswers,archetypeScores,adaptiveCandidates,tabPriority,deterministicProfile,classifyVisitor,
  answersFromLegacyQuery,intakeSchema,
  _test:{clamp01,completeness,harnessPick}
};
