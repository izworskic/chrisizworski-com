...
function candidateCompact(v,places,overnight,profile){
  ...
  const stops=[
    ...
    v.history>=.58?coreStop("fort","Fort Mackinac","anchor",profile.mobility==="limited"?"Strong history value, but treat the uphill approach as a mobility decision: favor carriage/taxi-style movement instead of forcing the climb.":"Strong first-visit/history value without crossing the whole Island."):null,
...
function candidateScenic(v,places,overnight,relaxedIntent){
  ...
  const score=scoreCandidate(46,[[v.crowd_avoidance,18],[v.special_occasion,17],[v.photography,15],[v.food,10],[relaxedIntent,8],[overnight?1:0,10]]);
...
function candidateActive(v,places,overnight,profile,bikeIntent,relaxedIntent){
  if(profile.mobility==="limited")return null;
  ...
  const score=scoreCandidate(43,[[v.outdoors,23],[bikeIntent,24],[v.photography,10],[relaxedIntent,-5],[v.kids_priority,3],[overnight?1:.45,5]]);
...
function candidateFamily(v,places,overnight,relaxedIntent){
  const meal=mealFor(places,"lunch")||mealFor(places,"dinner");
...
function buildCandidates({visitor={},places={},profile={}}={}){
  ...
  const bikeIntent=(profile.bikes&&profile.bikes!=="none")||interests.has("biking")?1:0;
  const pace=Number.isFinite(Number(v.pace))?Number(v.pace):.5;
  const relaxedIntent=clamp(1-pace,0,1);
  const list=[
    candidateCompact(v,places,overnight,profile),
    candidateScenic(v,places,overnight,relaxedIntent),
    candidateActive(v,places,overnight,profile,bikeIntent,relaxedIntent),
    candidateFamily(v,places,overnight,relaxedIntent),
