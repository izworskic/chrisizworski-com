...
function deterministicEdition(candidates,fallSnapshot,hold){
  if(!candidates.length){
    return {
  ...
  const weatherBits=[];
  if(rain!==null) weatherBits.push(`${Math.round(rain)} percent rain chance`);
  if(gust!==null) weatherBits.push(`gusts near ${Math.round(gust)} mph`);
  ...
  const conditions=weatherBits.length?` ${weatherBits.join(" and ")} keep the practical obstacles small.`:"";
  const second=next
...
function cleanEdition(raw,candidates,fallback){
  let parsed=null;
  try{
  ...
  const headline=safeText(parsed.headline,140);
  const read=safeText(parsed.read,2400);
  const notes={};
...
async function writeEditorial(candidates,fallSnapshot,hold){
  const fallback=deterministicEdition(candidates,fallSnapshot,hold);
  ...
  const key=`detroit-outdoors:edition:v2:${fp}`;
  try{
    const cached=await redis(["GET",key]);
    if(cached){
      const edition=cleanEdition(cached,candidates,fallback);
  ...
  const system=[
    "You are the writer of a small daily outdoors journal for Detroit and Southeast Michigan.",
    "The voice is plain, exact, unhurried, observant and local. Think field journal edited by a very good regional newspaper: restrained, specific, a little literary when the season gives you something real to notice.",
    "The reader is deciding whether to leave the house and where to spend a few hours. Write for that human decision, not for a software demo.",
    "Use weather and seasonal numbers as evidence inside sentences. Do not march through metrics. Translate them into what the day feels like and what they change.",
    "A calendar signal such as migration season is only timing. It does not prove birds were seen. A fall-color percentage is a model estimate, not an observation. Never invent crowds, sightings, trail condition, water safety, closures, events, or firsthand experience.",
    "Do not mention scores, models, APIs, JEV, the system, the site, the data stack, 'opportunities', 'weather fit', 'signal', 'current read', 'worth a closer look', or any other product language.",
    "Do not sound promotional. Do not write slogans. Avoid fragments unless they are genuinely natural. Do not write in first person. Do not use em dashes.",
    "The main read should be 130 to 190 words, in two short paragraphs. It should notice the season, make one clear lead choice, explain why that place fits this particular day, and place one alternative in context if useful.",
    "Each place note should be 35 to 65 words. Explain what kind of outing it is today and whether the drive makes sense. Do not repeat the same template across notes.",
    "Return JSON only, exactly as: {\"headline\":\"...\",\"read\":\"...\",\"notes\":{\"candidate-id\":\"...\"}}.",
    "Use only candidate IDs supplied in the input."
  ].join(" ");
  try{
    const response=await fetch("https://api.anthropic.com/v1/messages",{
      ...
      body:JSON.stringify({
        model:process.env.OUTDOORS_WRITER_MODEL||"claude-sonnet-4-6",
        max_tokens:900,
        temperature:.45,
        system,
        messages:[{role:"user",content:`Here are the verified facts for today's edition:\n${JSON.stringify(facts)}\n\nWrite the edition.`}]
      }),
    ...
    const edition=cleanEdition(raw,candidates,fallback);
    try{ await redis(["SET",key,JSON.stringify(edition),"EX",60*60*12]); }catch{}
    return {...edition,text:edition.read,mode:"anthropic",fingerprint:fp,model:process.env.OUTDOORS_WRITER_MODEL||"claude-sonnet-4-6"};
  }catch(error){
