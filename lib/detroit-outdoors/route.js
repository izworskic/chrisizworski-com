...
async function judgeEditorialTreatment(candidate,index,hold){
  ...
  return decideClosedSet({
    ...
    constraints:[
      ...
      "Use PLACE_CONTEXT when the main missing value is what is distinctive about this place or how to use it.",
...
function treatmentBrief(treatment){
  return {
    PLACE_CONTEXT:"Write 60 to 90 words that make this specific place more usable. Explain what kind of outing the place actually offers, which supplied physical features or trail/habitat facts matter for this activity, and how that changes the plan today.",
...
async function writeCardEditorial(candidate,slot,date){
  ...
  const key=`detroit-outdoors:card:v8:${fp}`;
  try{
    const cached=await redis(["GET",key]);
    if(cached){
      const parsed=JSON.parse(cached);
  ...
  try{
    ...
    if(!review.accepted){
      attempt=2;
      const repairPrompt=`${prompt}\n\nThe JEV reviewer rejected the first draft for this reason: ${safeText(review.reason,240)}. Rewrite it so it materially adds value, stays inside the supplied evidence, and directly answers the question. Return only JSON with note.`;
