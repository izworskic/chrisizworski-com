...
module.exports = async function detroitOutdoors(req,res){
  ...
  const mixed=dedupeMixedPool(parkSafePool,specialistGate.safe);
  ...
  res.status(200).json({
    ...
    sourceHealth:{
      ...
      greatLakesWater:{ok:Boolean(specialistStates.water&&specialistStates.water.ok),error:specialistStates.water&&specialistStates.water.error||null},
    ...
    notes:[
      ...
      "Park/weather, Great Lakes water, night-sky/aurora, and fall-color/phenology engines can emit normalized opportunity candidates. Deterministic safety and validity gates run before the mixed pool reaches JEV.",
