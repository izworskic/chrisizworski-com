...
const EDITORIAL_TREATMENTS = {
  ...
  SHIP_SIGHTING: "Translate a live AIS vessel report into a concise human ship-watching decision: what the current position and motion mean now, plus one verification step.",
...
function isFreighterCard(candidate){
  return Boolean(candidate&&(
    candidate.sourceEngine==="great-lakes-ais"||
    candidate.opportunityType==="live-freighter-passage"||
    (candidate.bundleSignals||[]).some(signal=>signal&&(
      signal.sourceEngine==="great-lakes-ais"||
      signal.opportunityType==="live-freighter-passage"
    ))
  ));
}
function fallbackTreatment(candidate,index){
  ...
  if(isFreighterCard(candidate)) return "SHIP_SIGHTING";
...
async function judgeEditorialTreatment(candidate,index,hold){
  const pureFreighter=isFreighterCard(candidate)&&!(Array.isArray(candidate.bundleSignals)&&candidate.bundleSignals.length>1);
  const options=pureFreighter
    ? {SHIP_SIGHTING:EDITORIAL_TREATMENTS.SHIP_SIGHTING}
  ...
  return decideClosedSet({
    ...
    constraints:[
      ...
      "Use SHIP_SIGHTING for a single live Great Lakes AIS / freighter card. It is a spotter brief, not a generic specialist-warning paragraph.",
...
function treatmentBrief(treatment){
  return {
    ...
    SHIP_SIGHTING:"Write 35 to 55 words as a ship-watching spotter brief. Lead with the named vessel and the practical meaning of its current AIS distance and reported motion. If speed is 0.5 kn or less, say this is not yet a chase-it-now passage signal and do not speculate why. End with one short check of the live tracker. Keep AIS limitations to one short clause. Never invent an ETA, waiting period, direction of travel, course change, visibility claim, or arrival time.",
...
function editorialQuestion(candidate,treatment){
  ...
  if(treatment==="SHIP_SIGHTING") return `What does this named vessel's current AIS distance and reported speed mean for someone deciding whether to head to the Detroit Riverfront now, and what single live-map check should they make before leaving?`;
...
function editorialEvidence(candidate,treatment,fallSnapshot){
  ...
  }else if(treatment==="SHIP_SIGHTING"){
    if(candidate.specialist) evidence.contextFacts.push(`Live AIS signal: ${candidate.specialist.headline}. ${candidate.specialist.detail||""}`);
    evidence.contextFacts.push("Detroit Outdoors ship-language rule: a report at 0.5 knots or less is a nearby-vessel lead, not an imminent-passage claim. Do not speculate why the vessel is slow or stopped.");
    evidence.contextFacts.push("Do not infer ETA, direction, a future Riverwalk segment, a waiting interval, or a course change unless those facts are explicitly present in the sealed evidence.");
    evidence.contextFacts.push(candidate.caveat);
...
async function judgeImageUnsafe(candidates){
  ...
  let result=await decideClosedSet({
    ...
    constraints:[
      ...
      "Every option in this pool is an approved historical file photo tied to a place on the selected board, so choose exactly one image.",
  ...
  if(result.mode!=="shared-harness-jev"&&pool.length>1){
    const retryOptions={};
    const retryMap={};
    for(const [index,image] of pool.entries()){
      const key=`IMAGE_${index+1}`;
      retryMap[key]=image;
      const matchedPlaces=board.filter(c=>(image.placeIds||[]).includes(c.place&&c.place.id)).map(c=>c.place.name);
      retryOptions[key]=`Historical file photo for ${matchedPlaces.join(", ")}. ${image.alt}. ${recentIds.includes(image.id)?"Recently used.":"Not recently used."}`;
    }
    const retryFallbackEntry=Object.entries(retryMap).find(([,image])=>image.id===freshFallback.id);
    const retryFallbackId=retryFallbackEntry&&retryFallbackEntry[0]||"IMAGE_1";
    const retry=await decideClosedSet({
      task:"Select exactly one hero image key for this Detroit Outdoors board. Return one of the supplied IMAGE_n keys. There is no abstain option.",
      options:retryOptions,
      context:{
        selectedBoardPlaces:board.map(c=>c.place&&c.place.name).filter(Boolean),
        selectedActivities:board.map(c=>c.activity).filter(Boolean)
      },
      constraints:[
        "Choose exactly one supplied IMAGE_n key.",
        "Do not answer NONE, abstain, skip, no-image, or any value not present in the options.",
        "Choose the most relevant photo for the selected board; use freshness only as a tiebreaker.",
        "These are historical file photos and are not evidence of current conditions."
      ],
      evidence:[],
      fallbackId:retryFallbackId,
      minConfidence:.52,
      timeoutMs:3000
    });
    const retryImage=retryMap[retry.choiceId]||null;
    if(retry.mode==="shared-harness-jev"&&retryImage){
      result={...retry,choiceId:retryImage.id,retry:true,originalMode:result.mode,originalReason:result.reason||null};
    }
  }
...
function fingerprint(candidates,fallSnapshot,hold,plan){
  ...
  const compact={
    ...
    c:candidates.slice(0,3).map(x=>{
      const weather=x&&x.weather||{};
      return {id:x&&x.id||null,s:finite(x&&x.score)===null?null:Math.round(x.score/5)*5,w:{h:round(weather.high,5),r:round(weather.rainChance,10),g:round(weather.gust,5),c:round(weather.cloudCover,10)}};
    }),
...
async function reviewEditorialNote(candidate,slot,note){
  ...
  const decision=await decideClosedSet({
    ...
    constraints:[
      ...
      "For SHIP_SIGHTING, reject prose that spends most of its space explaining AIS uncertainty instead of stating the vessel, current implication, and one next check.",
      "For SHIP_SIGHTING, reject invented waiting intervals, ETA/arrival claims, direction/course-change claims, visibility guarantees, or future Riverwalk-segment claims not present in the sealed evidence.",
...
function cardWriterSystem(){
  return [
    ...
    "For SHIP_SIGHTING, write like a concise local spotter alert, not an AIS disclaimer. Lead with the named vessel and the decision implication. Give at most one short AIS caveat.",
    "For SHIP_SIGHTING, if the sealed report is 0.5 knots or less, say only that it is not yet an active chase-it-now passage signal. Do not guess that the vessel is anchored, docked, stationary by intent, delayed, or suffering incomplete data.",
    "Never invent a wait-10-minutes rule, ETA, arrival time, direction, course change, visibility guarantee, or future viewing segment when the packet does not explicitly provide it.",
...
async function writeCardEditorial(candidate,slot,date,fallSnapshot){
  ...
  const key=`detroit-outdoors:card:v11:${fp}`;
  try{
    const cached=await redis(["GET",key]);
    if(cached){
      const parsed=JSON.parse(cached);
...
function intentTreatmentTitle(treatment,intentId){
  const byTreatment={
    SHIP_SIGHTING:"The ship-watcher read",
    MIGRATION_CONTEXT:"What migration means here",
    PLACE_CONTEXT:"How to use this place",
    WHY_TODAY:"Why this stands out today",
    DRIVE_DECISION:"Is the trip worth it?",
    NEXT_CHECK:"The check that still matters",
    SEASONAL_CONTEXT:"What the seasonal signal means"
  };
  if(byTreatment[treatment]) return byTreatment[treatment];
  return intentId==="freighter"?"The ship-watcher read"
    :intentId==="birding"?"The birder's read"
      :intentId==="water"?"The water-window read"
        :"The sunset read";
}
function intentMetricCards(candidate,intentId){
  if(!candidate) return [];
  const cards=[];
  const add=(label,value,detail="")=>{ if(value!==null&&value!==undefined&&String(value).trim()) cards.push({label,value:String(value),detail:safeText(detail,120)}); };
  if(candidate.timeWindow&&candidate.timeWindow.label) add("Best window",candidate.timeWindow.label,"Live engine window");
  if(intentId==="freighter"){
    const headline=candidate.specialist&&candidate.specialist.headline||"";
    const distance=headline.match(/([0-9]+(?:\.[0-9]+)?)\s*(?:mi|miles?)/i);
    const speed=headline.match(/([0-9]+(?:\.[0-9]+)?)\s*(?:kn|knots?)/i);
    if(distance) add("Reported distance",distance[1]+" mi","AIS-based");
    if(speed) add("Reported motion",speed[1]+" kn","AIS-reported speed");
  }else if(intentId==="water"){
    const headline=candidate.specialist&&candidate.specialist.headline||"";
    const waves=headline.match(/([0-9]+(?:\.[0-9]+)?)\s*ft/i);
    const wind=headline.match(/([0-9]+(?:\.[0-9]+)?)\s*mph/i);
    if(waves) add("Observed waves",waves[1]+" ft","Nearby NOAA/NDBC station");
    if(wind) add("Observed wind",wind[1]+" mph","Nearby NOAA/NDBC station");
  }else if(intentId==="sunset"){
    if(candidate.weather&&candidate.weather.cloudCover!==null&&candidate.weather.cloudCover!==undefined) add("Sky cover",Math.round(candidate.weather.cloudCover)+"%","Forecast, not a color guarantee");
    if(candidate.weather&&candidate.weather.rainChance!==null&&candidate.weather.rainChance!==undefined) add("Rain chance",Math.round(candidate.weather.rainChance)+"%","Current forecast");
  }else if(intentId==="birding"){
    if(candidate.place&&candidate.place.name) add("Best place",candidate.place.name,"Strongest hard-safe window");
    if(candidate.place&&candidate.place.drive) add("Drive band",candidate.place.drive,"Planning estimate");
  }
  if(candidate.confidence&&candidate.confidence.level) add("Confidence",candidate.confidence.level,candidate.confidence.reason||"");
  return cards.slice(0,4);
}
async function intentEditorial(intent,fallSnapshot){
  if(!intent||!intent.candidate) return null;
  const candidate=intent.candidate;
  const plan=await planEditorialPlacement([candidate],false,fallSnapshot);
  const slot=plan.cardNotes&&plan.cardNotes[0];
  if(!slot) return null;
  const date=new Intl.DateTimeFormat("en-US",{timeZone:"America/Detroit",weekday:"long",month:"long",day:"numeric",year:"numeric"}).format(new Date());
  const result=await writeCardEditorial(candidate,slot,date,fallSnapshot);
  return {
    candidateId:candidate.id,
    title:intentTreatmentTitle(result.treatment||slot.treatment,intent.id),
    treatment:result.treatment||slot.treatment,
    question:result.treatment?editorialQuestion(candidate,result.treatment):slot.question,
    note:result.note||null,
    mode:result.mode||"deterministic",
    model:result.model||null,
    attempt:result.attempt||null,
    plannerMode:plan.mode||"deterministic",
    plannerConfidence:slot.confidence||0,
    plannerReason:slot.reason||null,
    reassignedFrom:result.reassignedFrom||null,
    review:result.review||null,
    sources:result.sources||slot.evidence&&slot.evidence.sources||[]
  };
}
...
module.exports = async function detroitOutdoors(req,res){
  ...
  const requestedBoardIds=String(query.get("boardIds")||"").split(",").map(x=>x.trim()).filter(Boolean).slice(0,4);
  ...
  if(requestedIntent){
    ...
    const requestedCandidateId=query.get("candidateId");
    if(query.get("mode")==="editorial"){
      if(requestedCandidateId&&intent.candidate&&requestedCandidateId!==intent.candidate.id){
        res.status(409).json({ok:false,error:"intent-candidate-stale",requestedCandidateId,currentCandidateId:intent.candidate.id});
        return;
      }
      const enrichment=await intentEditorial(intent,fallSnapshot);
      res.status(200).json({
        ok:true,
        mode:"intent-editorial-enrichment",
        market:"Detroit / Southeast Michigan",
        ...
        intent:{id:intent.id,status:intent.status,candidateId:intent.candidate&&intent.candidate.id||null},
        enrichment
      });
      return;
    }
    res.status(200).json({
      ...
      intent:{
        ...intent,
        metrics:intentMetricCards(intent.candidate,intent.id),
        editorial:{mode:intent.candidate?"deferred":"not-applicable"}
      }
    });
    return;
  }
  ...
  if(query.get("mode")==="image"&&requestedBoardIds.length){
    ...
    const requestedBoard=requestedBoardIds.map(id=>byId.get(id)).filter(Boolean);
    if(requestedBoard.length!==requestedBoardIds.length){
      res.status(409).json({ok:false,error:"image-board-stale",requestedIds:requestedBoardIds,availableIds:boardPool.map(x=>x.id)});
      return;
    }
    const imageResult=await judgeImage(requestedBoard);
    res.status(200).json({
      ok:true,
      mode:"hero-image",
      market:"Detroit / Southeast Michigan",
      generatedAt:new Date().toISOString(),
      elapsedMs:Date.now()-started,
      runtime:{commitSha:process.env.VERCEL_GIT_COMMIT_SHA||null,vercelEnv:process.env.VERCEL_ENV||process.env.VERCEL_TARGET_ENV||null},
      boardIds:requestedBoard.map(x=>x.id),
      image:imageResult.selection,
      decision:{image:{mode:imageResult.decision.mode,choiceId:imageResult.decision.choiceId,confidence:imageResult.decision.confidence,reason:imageResult.decision.reason||null,pool:imageResult.pool||[],recentIds:imageResult.recentIds||[]}}
    });
    return;
  }

  if(query.get("mode")==="editorial"&&requestedBoardIds.length){
    const byId=new Map(boardPool.map(candidate=>[candidate.id,candidate]));
    const requestedBoard=requestedBoardIds.map(id=>byId.get(id)).filter(Boolean);
    if(requestedBoard.length!==requestedBoardIds.length){
      res.status(409).json({ok:false,error:"editorial-board-stale",requestedIds:requestedBoardIds,availableIds:boardPool.map(x=>x.id)});
      return;
    }
    const hold=query.get("hold")==="1";
    const ranked=requestedBoard.map((x,index)=>{
      const withSlot={...x,slot:deterministicLabel(index,x,requestedBoard[0]&&requestedBoard[0].score||0)};
