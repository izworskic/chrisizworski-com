...
function boardOption(candidate){
  const engine=candidate.sourceEngine||"unknown";
  ...
  return `${candidate.place.name}: ${candidate.opportunityType||candidate.activity}; source engine ${engine}; time window ${window}; heuristic evidence score ${candidate.score}/100; drive planning band ${candidate.place.drive}; why now: ${why}.`;
}
...
module.exports = async function detroitOutdoors(req,res){
  ...
  const boardDecision=await editBoard(safePool,4);
  ...
  if(query.get("mode")==="image"){
    let imageBoard=hold?[]:ranked;
    if(requestedBoardIds.length){
      const byId=new Map(safePool.map(candidate=>[candidate.id,candidate]));
      const resolved=requestedBoardIds.map(id=>byId.get(id)).filter(Boolean);
      if(resolved.length!==requestedBoardIds.length){
        res.status(409).json({ok:false,error:"stale-board",requestedBoardIds,availableIds:safePool.map(x=>x.id)});
        return;
      }
      imageBoard=resolved;
    }
    ...
    res.status(200).json({
      ...
      decision:{
        ...
        boardEditor:{
          ...
          candidateCountByEngine:countByEngine(safePool),
          selectedCount:ranked.length,
          selectedIds:ranked.map(x=>x.id),
          selectedEngineDiversity:new Set(ranked.map(x=>x.sourceEngine).filter(Boolean)).size,
