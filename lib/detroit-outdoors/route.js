...
function fallbackTreatment(candidate,index){
  ...
  if(hasMigrationSignal(candidate)) return "MIGRATION_CONTEXT";
...
async function judgeEditorialTreatment(candidate,index,hold){
  ...
  return decideClosedSet({
    ...
    constraints:[
      ...
      "Use MIGRATION_CONTEXT when migration timing is a material reason for a birding card and the reader would otherwise not know what migration means here.",
...
function treatmentBrief(treatment){
  return {
    WHY_TODAY:"Write 50 to 80 words that explain the non-obvious reason this deserves attention today. Synthesize verified live conditions with useful place context instead of paraphrasing the card.",
    DRIVE_DECISION:"Write 55 to 85 words that resolve the travel tradeoff. Explain what the user gains for the drive, what weakens the case, and who should actually make the trip today. Use place context when supplied.",
    NEXT_CHECK:"Write 45 to 70 words around the unresolved specialist gate. Explain what is already encouraging, what could still flip the decision, and exactly what the user should verify before committing.",
    MIGRATION_CONTEXT:"Write 75 to 110 words that make the migration signal understandable. Explain why this place matters during migration, which bird groups or habitats are relevant from the supplied evidence, what the current date/season changes, and what is still unknown without a live sightings check. Make this genuinely educational and useful, not a generic birding sentence.",
    SEASONAL_CONTEXT:"Write 55 to 85 words that translate the supplied seasonal signal into realistic expectations for this outing. Explain what the timing changes and what it does not prove at this exact place."
  }[treatment]||"";
}
...
async function writeEditorial(candidates,fallSnapshot,hold,plan){
  ...
  if(!process.env.ANTHROPIC_API_KEY) return {...fallback,text:fallback.read,mode:"deterministic",reason:"missing ANTHROPIC_API_KEY"};
  if(!candidates.length) return {...fallback,text:fallback.read,mode:"deterministic",reason:"no candidates"};
  const fp=fingerprint(candidates,fallSnapshot,hold,plan);
  const key=`detroit-outdoors:edition:v7:${fp}`;
  try{
  ...
  const facts={
    ...
    cardBriefs:((plan&&plan.cardNotes)||[]).map(x=>({candidateId:x.candidateId,treatment:x.treatment,placement:x.placement,question:x.question,brief:x.brief,additiveEvidence:x.evidence})),
  ...
  const system=[
    ...
    "When additiveEvidence is supplied, use it. That evidence exists specifically because the classifier determined the card needs context the visible card does not contain.",
    "For MIGRATION_CONTEXT, explain the place and habitat significance, likely bird groups only when explicitly supplied, seasonal timing, and the limitation that this is not a same-day sighting report.",
    "If you cannot add material value beyond visibleCard and additiveEvidence, omit that candidate from notes instead of filling space.",
