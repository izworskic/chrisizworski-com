...
module.exports = async function detroitOutdoors(req,res){
  ...
  const requestedImageBoardIds=String(query.get("boardIds")||"").split(",").map(x=>x.trim()).filter(Boolean).slice(0,4);
  ...
  if(query.get("mode")==="image"&&requestedImageBoardIds.length){
    const byId=new Map(boardPool.map(candidate=>[candidate.id,candidate]));
    const requestedBoard=requestedImageBoardIds.map(id=>byId.get(id)).filter(Boolean);
    if(requestedBoard.length!==requestedImageBoardIds.length){
      res.status(409).json({ok:false,error:"image-board-stale",requestedIds:requestedImageBoardIds,availableIds:boardPool.map(x=>x.id)});
      return;
    }
