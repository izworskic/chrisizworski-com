...
async function recentImageHistory(){
  try{
    const raw=await redis(["GET","detroit-outdoors:image-history:v1"]);
...
async function rememberImage(image){
  ...
  try{
    ...
    await redis(["SET","detroit-outdoors:image-history:v1",JSON.stringify(next),"EX",60*60*24*10]);
  }catch{}
}
async function judgeImage(candidates){
  ...
  options.NONE="Use no image if every available file photo would misrepresent today's selected opportunities.";
  ...
  const result=await decideClosedSet({
    ...
    constraints:[
      ...
      "Choose NONE if the available photos would imply a current condition not established by the board."
    ],
  ...
  if(selected) await rememberImage(selected);
