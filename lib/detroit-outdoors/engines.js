...
function freighterWatchingCandidate({placeStates,alertStates,aisState}){
  ...
  const speed=finite(vessel.speedKnots);
  const activePassage=speed!==null&&speed>0.5;
  const motionAdjustment=speed===null?-3:speed<=0.5?-12:speed<3?-3:speed>=5?3:0;
  const score=clamp(Math.round((distance<=2?91:distance<=5?86:distance<=8?80:74)+motionAdjustment));
  return [normalizeOpportunityCandidate({
    ...
    title:activePassage?"Detroit River live freighter window":"Detroit River possible freighter sighting",
    score,
    ...
    reasons:[
      ...
      speed===null?"speed unavailable":`reported speed ${speed.toFixed(1)} kn`,
      activePassage?"AIS indicates active movement":"AIS does not currently show active passage speed",
    ...
    specialist:{
      ...
      headline:`${safeText(vessel.name,100)} · ~${distance} mi from Detroit Riverfront${speed===null?"":` · ${speed.toFixed(1)} kn`}`,
      detail:activePassage
        ?"Fresh AIS places a moving commercial vessel near the Detroit River. Exact passage timing is still not guaranteed."
        :`Fresh AIS places a commercial vessel nearby, but its reported speed is ${speed===null?"unavailable":`${speed.toFixed(1)} kn`}. Treat this as a possible sighting lead, not an imminent passage.`,
    ...
    standout:distance<=6&&activePassage,
    verifyUrl:"https://chrisizworski.com/great-lakes-freighter-tracking/",
    caveat:"AIS is informational, not a passage schedule; verify the live map before leaving.",
    timeWindow:{label:activePassage?"Possible passage now / verify live map":"Nearby vessel / movement not confirmed",start:vessel.seen||null,end:null},
    whyNow:activePassage
      ?`A fresh AIS report places ${safeText(vessel.name,100)} about ${distance} miles from the Detroit Riverfront and shows it moving at ${speed.toFixed(1)} kn, creating a possible ship-watching window if the live map still confirms the track.`
      :`A fresh AIS report places ${safeText(vessel.name,100)} about ${distance} miles from the Detroit Riverfront, but the reported speed is ${speed===null?"unavailable":`${speed.toFixed(1)} kn`}; this is a nearby-vessel lead rather than a confirmed passage window.`,
    verifiedEvidence:[
      {source:"Great Lakes Ship Tracker",sourceLabel:"Fresh AIS vessel report",sourceUrl:"https://chrisizworski.com/great-lakes-freighter-tracking/",text:`${safeText(vessel.name,100)} reported at ${safeText(vessel.seen,80)}, about ${distance} miles from the Detroit Riverfront reference point${speed===null?"":`, speed ${speed.toFixed(1)} kn`}.`},
    ...
    confidence:{level:distance<=5&&activePassage?"high":"medium",reason:activePassage?"Fresh normalized AIS position with active reported movement; exact passage timing remains uncertain.":"Fresh normalized AIS position, but active passage movement is not currently established."},
    hardStops,
    ...
    travel:travelPacket(DETROIT_RIVERFRONT),
    uncertainty:[
      "AIS does not guarantee visibility from a specific Riverwalk segment.",
      activePassage?"Exact passage timing can change.":"Current AIS speed does not support an imminent-passage claim."
    ]
  })];
}
...
function bundleCandidatesByPlace(candidates){
  ...
  for(const rows of groups.values()){
    const strongestByEngine=new Map();
    for(const row of rows){
      const key=row&&row.sourceEngine||row&&row.id||"unknown";
      const prior=strongestByEngine.get(key);
      if(!prior||(Number(row.score)||0)>(Number(prior.score)||0)) strongestByEngine.set(key,row);
    }
    const sorted=[...strongestByEngine.values()].sort((a,b)=>(Number(b.score)||0)-(Number(a.score)||0));
