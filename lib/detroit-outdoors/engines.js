...
async function loadOne(url,name){
  try{
...
async function loadSpecialistEngineStates(){
  const [water,nightSky]=await Promise.all([
    loadOne(BUOY_API,"great-lakes-water"),
    loadOne(AURORA_API,"night-sky-aurora")
  ]);
...
function waterCandidate({placeStates,alertStates,waterState}){
  ...
  const marineHazard=matchingAlert(alertState,WATER_HARD_ALERT);
  const severeHazard=matchingAlert(alertState,HARD_WEATHER_ALERT);
  const hardStops=[];
  if(observedAge===null||observedAge>4) hardStops.push("Required NOAA/NDBC water observation is older than four hours.");
  if(marineHazard) hardStops.push(`Active marine/shoreline hazard: ${safeText(marineHazard.event,100)}.`);
...
function nightSkyCandidate({placeStates,alertStates,nightSkyState}){
  ...
  return [normalizeOpportunityCandidate({
    ...
    weather:{
      high:weather&&weather.high===null?null:weather&&Math.round(weather.high),
      low:weather&&weather.low===null?null:weather&&Math.round(weather.low),
      rainChance:weather&&weather.rainChance===null?null:weather&&Math.round(weather.rainChance),
      gust:weather&&weather.gust===null?null:weather&&Math.round(weather.gust),
      cloudCover:window.cloudPercent,
      aqi:weather&&weather.aqi===null?null:weather&&Math.round(weather.aqi)
    },
    specialist:{
...
function engineDiagnostics({parkCandidates,emitted,gated,mixedPool,selected,specialistStates,replaced}){
  ...
  const sourceStatus={
    "great-lakes-water":{ok:Boolean(specialistStates&&specialistStates.water&&specialistStates.water.ok),error:specialistStates&&specialistStates.water&&specialistStates.water.error||null},
