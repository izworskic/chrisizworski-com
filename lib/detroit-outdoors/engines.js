...
async function loadSpecialistEngineStates(){
  const [water,nightSky,waterMarineAlerts,freighterAis]=await Promise.all([
  ...
  return {water,nightSky,freighterAis};
}
...
function emitSpecialistCandidates({placeStates,alertStates,fallSnapshot,specialistStates}){
  ...
  return {
    ...
    candidates:[...water,...nightSky,...fallColor,...sunset,...freighter]
  };
}
...
function engineDiagnostics({parkCandidates,emitted,gated,mixedPool,selected,specialistStates,replaced,vetoedLegacy}){
