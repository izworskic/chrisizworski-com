...
function signalEligible(candidate,now=new Date()){
  ...
  if(NIGHT_ENGINES.has(engine)||NIGHT_ACTIVITIES.has(activity))return true;
  if(engine==="sunset-photography")return end===null||now.getTime()<=end;
...
async function detroitDecideClosedSet(args){
  ...
  if(/overall posture/i.test(String(args.task||""))&&!filtered.eligibleEvidence.length){
    return{mode:"deterministic",choiceId:"QUIET",confidence:1,reason:"No daylight-eligible or night-capable candidates remain after deterministic time gating",auth:"pre-jev-daylight-gate"};
  }
  if(!candidateIds.length&&optionIds.includes("STOP")){
    return{mode:"deterministic",choiceId:"STOP",confidence:1,reason:"No eligible candidate remains after deterministic time gating",auth:"pre-jev-daylight-gate"};
  }
...
function filterResponse(body,now=new Date(),state={preJevFiltered:new Set()}){
  ...
  return{
    ...body,
    opportunities:kept,
    verdict:changed&&!kept.length?{label:"QUIET",detail:"Daylight-dependent outings are closed for the evening. Only opportunities that remain usable now are eligible for this board."}:body.verdict,
    frontPage:changed?{...(body.frontPage||{}),subhead:kept.length?body.frontPage&&body.frontPage.subhead||"Only currently usable opportunities remain on the board.":"Daylight-dependent outings have closed for the evening; the board is holding unless a genuine night-capable opportunity clears its live gates."}:body.frontPage,
...
module.exports._test={driveMinutes,candidateCoords,signalEligible,candidateEligible,parseEvidenceCandidate,filterClosedSetArgs,filterResponse,isBoardDecisionTask};
