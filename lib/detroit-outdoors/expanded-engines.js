...
async function loadExpandedEngineStates(){
  const [birdMigration,beach,ice,morel,xc,rivers]=await Promise.all([
    loadOne(BIRD_MIGRATION_API,"bird-migration-live",11000),
    loadOne(BEACH_API,"great-lakes-beach",13000),
    loadOne(ICE_API,"lake-st-clair-ice",12000),
    loadOne(MOREL_API,"morel-phenology",13000),
    loadOne(XC_MODEL_API,"xc-snow-screen",13000),
...
function expandedSourceStatus(states){
  ...
  return Object.fromEntries(Object.entries(ids).map(([key,id])=>[id,{ok:Boolean(states&&states[key]&&states[key].ok),error:states&&states[key]&&states[key].error||null}]));
}
...
module.exports={
  ...
  _test:{birdCandidate,beachCandidate,riverCandidate,morelCandidate,cleanAirCandidate,iceCandidate,xcCandidate,parseRiverSeries,median}
};
