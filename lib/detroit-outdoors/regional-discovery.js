...
function candidateFrom(place,weather,alertState,targetDate){
  ...
  const strengths=[];
  if(finite(weather.precipitationProbability)!==null&&weather.precipitationProbability<=35)strengths.push("low rain risk");
  if(finite(weather.windGust)!==null&&weather.windGust<=24)strengths.push("manageable wind");
  if(finite(weather.high)!==null&&weather.high>=45&&weather.high<=82)strengths.push("comfortable daytime temperatures");
  if(finite(weather.cloudCover)!==null&&weather.cloudCover<=40)strengths.push("brighter skies");
  const conditionRead=strengths.length?strengths.slice(0,2).join(" and "):"a usable weather window";
  const minutes=finite(place&&place.driveMinutes);
  const driveRead=minutes===null?"within the Detroit-area search radius":`about ${Math.round(minutes)} minutes from central Detroit`;
  const categoryRead=safeText(place.categoryLabel||place.category||"outdoor destination",80).toLowerCase();
  const activityRead=activity==="hiking"?"a walk or hike":activity==="birding"?"birding":"a scenic outing";
  return {
    ...
    whyNow:`${shaped.name} is a ${categoryRead} ${driveRead}. Today's point forecast has ${conditionRead}, making it a practical option for ${activityRead}. Check current access or closures before leaving.`,
    verifiedEvidence:[
