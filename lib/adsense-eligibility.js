...
function placerTag(pathname = '', settings = inArticle) {
  ...
  return `<script defer src="${PLACER_SRC}?v=1" data-client="${settings.publisherId}" data-slot="${settings.slotId}" data-max="${Number(settings.maxPerPage) || 3}"></script>`;
}
const hasPlacer = html => html.includes(`src="${PLACER_SRC}`);
module.exports = { eligible, removeAdLoader, normalizeAdLoader, placerTag, hasPlacer, LOADER_SRC, LOADER_TAG };
